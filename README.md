...
- 구조 189/189문항 — 「문제 → 대안 → 기준 → 결론 → 비용 → 측정」 여섯 줄을 맨 위에 두고 핵심 답은 그 이유로 배치
...
**문제** — 함수 호출 사이에 값을 유지하면서 밖에서는 못 건드리게 해야 한다(카운터·타이머 id·once 플래그).

**대안** — 전역/모듈 변수는 노출되고 인스턴스를 여럿 못 만든다. 클래스 프라이빗 필드는 여러 메서드가 상태를 공유할 때. WeakMap 은 이미 객체가 있을 때.

**기준** — 호출 사이에 값을 살려 두되 누가 접근할 수 있게 할지(노출 범위)와 그 값을 얼마나 오래 붙잡는지로 본다.

**결론** — 클로저는 함수가 태어난 자리의 변수를 기억해 호출 사이 값을 유지하면서 밖에서는 못 건드리게 한다. 대가는 그 변수가 함수 수명만큼 남는 것.

**비용** — 변수 수명이 함수 수명만큼 늘어난다(메모리). 상태가 보이지 않아 디버깅이 어렵고, 큰 객체를 붙잡으면 누수.

**측정** — 힙 스냅샷 전후 비교로 남은 객체와 retainer 확인.
...
**문제** — 다른 타입의 값을 비교할 때 결과가 예측 가능해야 한다.

**대안** — == 로 통일하면 변환 규칙을 전부 외워야 하고, Object.is 는 NaN·±0 만 다르게 취급하는 특수 용도, 라이브러리 isEqual 은 깊은 비교라 목적이 다르다.

**기준** — 비교 전에 타입 변환이 일어나는가로 나눈다.

**결론** — 기본은 ===. == 는 null/undefined 를 한 번에 잡는 x == null 한 가지만 허용한다.

**비용** — === 로 통일하면 명시적 변환 코드가 늘어난다. x == null 관용구 하나는 예외로 남긴다.

**측정** — ESLint eqeqeq 규칙으로 위반을 잡고, 변환 의도가 있는 곳은 Number()/String() 으로 드러난다.
...
**문제** — 변수가 어디서 보이고 언제부터 쓸 수 있는지 예측 가능해야 한다.

**대안** — var 하나로 쓰면 호이스팅·함수 스코프 때문에 루프·조건문에서 값이 새어 나온다.

**기준** — 스코프 범위와 초기화 시점(호이스팅·TDZ)으로 나눈다.

**결론** — var 는 함수 스코프·undefined 초기화, let/const 는 블록 스코프·TDZ. const 는 재할당만 막고 내용은 바뀐다.

**비용** — let/const 는 TDZ 때문에 선언 전 접근이 에러다 — 선언 순서를 신경 써야 한다.

**측정** — no-var·prefer-const 린트 통과, 선언 전 접근이 ReferenceError 로 즉시 드러난다.
...
**문제** — 비동기 콜백들의 실행 순서를 알아야 화면 갱신 타이밍과 경합을 설계할 수 있다.

**대안** — 순서에 기대지 않고 await 로 명시적으로 직렬화하는 것이 안전한 대안이다. 그래도 렌더 타이밍·마이크로태스크 기아를 설계하려면 큐 구조를 알아야 한다.

**기준** — 큐가 몇 종류이고 우선순위가 어떻게 다른가로 나눈다.

**결론** — 마이크로태스크 큐를 전부 비운 뒤 렌더 기회, 그다음 매크로태스크 하나 — 그래서 then·queueMicrotask 가 setTimeout 보다 먼저다.

**비용** — 마이크로태스크를 남용하면 렌더가 굶는다. 프레임에 양보하는 코드가 필요하다.

**측정** — console.log 순서 실험으로 재현, Performance 패널에서 Long Task 와 빈 프레임 확인.
...
**문제** — 메서드를 콜백으로 넘겼을 때도 원래 객체를 가리켜야 한다.

**대안** — 매번 bind 하면 호출부가 지저분하고, 화살표만 쓰면 프로토타입 메서드·이벤트 currentTarget 에서 문제.

**기준** — this 를 정하는 것이 정의 위치인가 호출 형태인가로 나눈다.

**결론** — 일반 함수는 호출 형태가, 화살표는 정의 위치가 정한다. 콜백으로 떼어 넘기면 호출 형태가 바뀌어 유실된다.

**비용** — 클래스 필드 화살표·bind 는 인스턴스마다 함수를 새로 만든다(메모리).

**측정** — this 유실은 undefined 접근 에러로 드러나고, 단위 테스트에서 콜백 전달 케이스를 고정한다.
...
**문제** — 상태를 바꿀 때 원본을 오염시키지 않으면서 비용을 최소화해야 한다.

**대안** — 항상 깊은 복사(JSON 왕복)는 손실과 비용, 항상 얕은 복사는 중첩 오염.

**기준** — 얼마나 깊이 복사해야 하는가, 손실이 허용되는가, 갱신 빈도는 어떤가로 나눈다.

**결론** — 보통은 복사 대신 바뀐 경로만 새로 만든다. 진짜 스냅샷이 필요할 때만 structuredClone, JSON 왕복은 손실이 있다.

**비용** — 불변 갱신은 경로마다 새 객체를 만드는 코드가 늘어난다(Immer 로 완화).

**측정** — 참조 비교(===)로 갱신 여부 검증, 오염은 원본 스냅샷 대조 테스트로.
...
**문제** — 여러 비동기 작업의 결과를 화면 요구(부분 실패 허용 여부)에 맞게 모아야 한다.

**대안** — all 하나로 다 처리하면 위젯 하나 실패에 화면 전체가 죽는다. 수동 try/catch 반복은 누락이 생긴다.

**기준** — 부분 실패를 화면에 보여야 하는가, 전부 성공해야 의미 있는가로 나눈다.

**결론** — 부분 실패 허용이면 allSettled, 트랜잭션형이면 all. 취소는 조합자가 아니라 AbortSignal 이 한다.

**비용** — allSettled 는 실패 결과를 각각 처리하는 UI 코드가 필요하고, 취소는 별도(AbortSignal)로 붙여야 한다.

**측정** — 네트워크 차단 주입으로 부분 실패 화면을 e2e 로 고정, 타임아웃 후 요청이 pending 인지 네트워크 패널로.
...
**문제** — 실패가 조용히 사라지지 않고 반드시 어딘가에 보고되어야 한다.

**대안** — try/catch 만 믿으면 await 누락·forEach·타이머 경로를 놓친다.
...
**비용** — 전역 핸들러는 노이즈가 섞이니 fire-and-forget 에도 .catch(report) 를 붙이는 규율이 필요하다.

**측정** — unhandledrejection 카운터·에러 트래킹 수치, 'Pause on uncaught exceptions' 로 발생 지점.
...
**문제** — 객체에 부가 정보를 붙이되 그 객체의 수명을 늘리면 안 된다.

**대안** — 강한 Map 은 노드 제거 후에도 회수되지 않는다. 객체에 직접 속성을 붙이면 외부 객체를 오염시킨다.

**기준** — 부가 정보를 붙일 객체의 수명을 누가 소유하는가로 나눈다.

**결론** — 객체 수명을 늘리지 않아야 하면 WeakMap. 단 회수 시점은 보장되지 않으니 정리 로직의 유일한 수단으로 쓰지 않는다.

**비용** — WeakMap 은 순회·크기 조회가 안 되고, WeakRef 회수 시점은 보장되지 않는다.

**측정** — 힙 스냅샷에서 Detached 노드·retainer 확인.
...
**문제** — 값 공유로 인한 의도치 않은 변경을 막아야 한다.

**대안** — 전부 깊은 복사하면 비용이 크고, Object.freeze 는 얕고 개발 모드용이다. 경계에서만 복사하고 내부는 불변 갱신 규칙으로 간다.
...
**문제** — '없음'과 '비움'을 혼동하지 않아야 한다.

**대안** — 둘을 같게 보면 API 계약이 흔들린다.

**기준** — '아직 없음'인가 '비어 있음을 명시'인가로 나눈다.

**결론** — undefined 는 미할당, null 은 의도적 비움. API 계약에서 하나로 고정하고 기본값은 ?? 로.

**비용** — API 계약에서 '필드 생략'과 'null' 의 의미를 문서로 고정하고, 클라이언트 타입에 그대로 반영해야 한다.

**측정** — null/undefined 처리 테스트.
...
**문제** — 없는 값 접근 에러를 막되 결함을 숨기지 않아야 한다.

**대안** — 전부 ?. 는 원인 은폐, || 는 0 덮기.

**기준** — 값이 실제로 없을 수 있는 자리인가로 본다.

**결론** — 있을 수 있는 값에만 ?. 를 쓰고 필수 값은 경계에서 검증한다. || 는 0·'' 을 덮으니 ?? 를 쓴다.

**비용** — 필수 값은 경계에서 스키마로 검증하는 코드가 필요하다. ?. 로 넘기던 곳을 걷어내면 잠깐 에러가 늘어난다(원래 있던 결함이 드러나는 것).

**측정** — 필수 값 누락이 경계 검증 로그로 잡히는지, ?. 로 가려져 있던 undefined 렌더가 사라졌는지.
...
**문제** — 가독성과 성능을 함께 만족하는 순회를 골라야 한다.

**대안** — 항상 고차 함수/항상 for 는 둘 다 틀림.

**기준** — 가독성이 먼저인가 규모가 먼저인가로 나눈다.

**결론** — 의도가 이름으로 드러나면 고차 함수, 중간 탈출·복잡 누적은 for. 수만 건이면 한 루프로 합친다.

**비용** — 팀이 '읽기 쉬운 순회'의 기준을 합의해야 하고, 대량 데이터 구간은 벤치 결과를 근거로 예외를 둔다.
...
**문제** — 첫 화면이 어디서 막히는지 알아야 무엇을 고칠지 정해진다.

**대안** — '번들을 줄이자'만 하면 CSS 블로킹·TTFB 가 원인일 때 효과가 없다.

**기준** — 각 단계에서 무엇이 렌더를 막는가(파서 블로킹 vs 렌더 블로킹)로 나눈다.

**결론** — 스크립트는 파서를, CSS 는 렌더를 막는다. 첫 화면 개선은 지배 구간을 측정해 그 블로킹부터 푼다.

**비용** — 단계별 최적화(defer·인라인 CSS·preload)는 각각 캐시·HTML 크기 같은 부작용이 있다.

**측정** — Performance 패널의 Parse HTML 끊김, Coverage 미사용 CSS, LCP 구간 분해.
...
**문제** — 애니메이션·스크롤 중 프레임을 지켜야 한다.

**대안** — 모든 속성 변경을 같은 것으로 보면 top/width 애니메이션이 매 프레임 레이아웃을 부른다.

**기준** — 속성이 파이프라인의 어느 단계까지 다시 실행시키는가로 나눈다.

**결론** — 레이아웃 > 페인트 > 컴포지트 순으로 비싸다. 애니메이션은 transform/opacity 로, 읽기와 쓰기는 분리한다.

**비용** — transform/opacity 로 옮기면 레이어가 늘어 GPU 메모리를 쓰고, will-change 남용은 역효과.

**측정** — Performance 의 Layout 반복·Forced reflow 경고, Layers 패널.
...
**문제** — 동적으로 늘어나는 요소들의 이벤트를 리스너 폭발 없이 처리해야 한다.

**대안** — 요소마다 리스너를 붙이면 추가·삭제마다 재바인딩과 누수. stopPropagation 으로 막는 건 다른 기능을 죽인다.

**기준** — 이벤트가 어느 방향으로 흐르고 어디서 잡는가로 본다.

**결론** — 캡처→타깃→버블. 동적 목록은 상위 하나에서 위임하고 closest 로 판별한다. stopPropagation 은 다른 기능을 깨뜨린다.

**비용** — 위임은 target 판별 코드(closest)가 필요하고, 위임 못 하는 이벤트(focus 등)는 별도 처리.

**측정** — 리스너 수·이벤트 도달 여부를 DevTools Event Listeners 패널과 e2e 로.
...
**문제** — 장시간 사용에도 메모리가 계속 늘지 않아야 한다.

**대안** — 'GC 가 알아서'는 참조가 남은 객체엔 적용되지 않는다. 코드 리뷰만으로는 못 잡는다.

**기준** — 누수를 추정하나 스냅샷으로 증명하나로 갈린다.

**결론** — 힙 스냅샷 2회 비교로 증가와 retainer 를 확정한다. 원인은 리스너·타이머·전역 캐시·detached DOM 네 갈래.

**비용** — 클린업 코드·해제 함수 반환이 늘어나고, 감시 시나리오를 유지해야 한다.

**측정** — 힙 스냅샷 2회 비교, 반복 시나리오 후 JS heap 추세, Detached DOM 검색.
...
**문제** — 시각 갱신을 화면 주기에 맞춰 끊김 없이 해야 한다.

**대안** — setTimeout 으로 애니메이션하면 프레임과 어긋나고 배경 탭에서도 돈다.

**기준** — 프레임과 동기화되는가, 배경 탭에서 멈추는가로 본다.

**결론** — 애니메이션은 rAF, 폴링은 타이머. 프레임 예산 16ms 를 넘는 작업은 쪼갠다.

**비용** — rAF 는 배경 탭에서 멈추므로 타이머 성격의 작업엔 부적합하다.

**측정** — 프레임 타임라인에서 드랍 프레임, 주사율 다른 기기에서 속도 검증(delta 기반인지).
...
**문제** — 무거운 계산 중에도 입력에 응답해야 한다.

**대안** — 메인 스레드에서 setTimeout 으로만 쪼개면 총 시간이 늘고, 서버 이관은 왕복이 지배할 수 있다.

**기준** — 작업을 다른 스레드로 옮길 수 있는가, 데이터 전달 비용이 얼마인가로 나눈다.

**결론** — DOM 결합이 없으면 워커 + Transferable, 있으면 청크 분할 + yield. 어느 쪽이든 Long Task 로 측정한다.

**비용** — 워커는 메시지 직렬화·프로토콜·취소·실패 처리 코드가 필요하다.

**측정** — Long Task 개수·INP, 워커 왕복 시간을 performance.mark 로.
...
**문제** — 크기 지정이 계산대로 나오고 간격이 예측 가능해야 한다.

**대안** — content-box 그대로 두면 padding 을 더한 폭을 매번 계산해야 한다.

**기준** — width 가 콘텐츠만 뜻하나 테두리까지 뜻하나로 나눈다.

**결론** — border-box 로 리셋해 계산을 예측 가능하게 한다. 수직 마진 병합은 flex/grid 컨테이너에서 사라진다.

**비용** — border-box 리셋은 서드파티 CSS 와 충돌할 수 있다.

**측정** — DevTools Computed 의 박스 모델, 레이아웃 스냅샷 테스트.
...
**문제** — 겹치는 요소의 위아래를 의도대로 제어해야 한다.

**대안** — z-index 값을 키우는 건 부모 맥락 밖으로 못 나가니 해결이 아니다.

**기준** — z-index 가 같은 쌓임 맥락 안에서 경쟁하는가로 본다.

**결론** — 부모가 맥락을 만들면 자식 값은 그 안에서만 의미다. 값을 올리지 말고 DOM 위치나 포털로 푼다.

**비용** — 포털은 포커스·스크롤·컨텍스트 연결을 다시 챙겨야 한다. 레이어 토큰 관리가 필요하다.

**측정** — DevTools Layers 패널로 쌓임 맥락 확인, 드롭다운·모달 겹침 e2e.
...
**문제** — 한 축 정렬과 두 축 배치를 각각 맞는 도구로 해야 한다.

**대안** — float·table 은 반응형·정렬 제어가 약하고, grid 만 쓰면 단순 정렬이 과하다.

**기준** — 한 축 흐름인가 두 축 배치인가로 나눈다.

**결론** — 1축 정렬은 flex, 2축 트랙은 grid. flex 가 안 줄어들면 min-width:auto 가 원인이다.

**비용** — flex 는 min-width:auto 함정, grid 는 브라우저 지원과 학습 비용.

**측정** — 400px·1200px 폭 스냅샷, 긴 텍스트 넘침 테스트.
...
**문제** — sticky 를 적용했는데 헤더가 붙지 않는 원인을 찾아야 한다.

**대안** — fixed 는 문서 흐름에서 빠져 공간 보정이 필요하다. JS 스크롤 계산은 성능·정확도가 나쁘다.

**기준** — 스크롤 컨테이너가 어디인가(조상 overflow)로 먼저 본다.

**결론** — 조상 overflow → 임계값 미지정 → 부모 높이 부족 순으로 확인한다. 브라우저 버그가 아니다.

**비용** — 조상 overflow 제약 때문에 레이아웃 구조를 바꿔야 할 수 있다.

**측정** — DevTools 로 조상 overflow 확인, 표 헤더 스크롤 e2e.
...
**문제** — 부모·자식 크기를 모를 때도 정확히 중앙에 둬야 한다.

**대안** — margin 계산·line-height 트릭은 크기가 바뀌면 깨진다.

**기준** — 부모 크기를 아는가, 자식 크기를 아는가로 나눈다.

**결론** — 모르면 grid/flex 정렬 또는 translate 50%. 뷰포트 높이는 100vh 대신 dvh.

**비용** — absolute+translate 는 문서 흐름에서 빠져 주변에 공간을 만들지 않는다 — 감싸는 레이아웃이 높이를 따로 확보해야 한다.

**측정** — 여러 뷰포트·글자 크기에서 스냅샷.
...
**문제** — 다크 모드·테마를 컴포넌트 수정 없이 바꿀 수 있어야 한다.

**대안** — 컴포넌트마다 .dark 분기를 넣으면 색이 흩어져 대비 사고가 난다.

**기준** — 색을 누가 정의하고 누가 참조하는가로 나눈다.

**결론** — 토큰은 :root 에 전량, 다크는 토큰만 재정의, 컴포넌트는 토큰만 참조. 미디어 쿼리 안에만 있는 색은 사고다.

**비용** — 토큰 계층 설계와 기존 리터럴 색 마이그레이션 비용.

**측정** — 대비(WCAG) 자동 검사, 세 상태(시스템/명시 라이트/명시 다크) 시각 회귀.
...
**문제** — 스타일 충돌을 예측 가능하게 풀어야 한다.

**대안** — !important 남발은 다음 사람도 남발.
...
**문제** — 요소 표시 방식이 레이아웃·접근성 의도와 맞아야 한다.

**대안** — none 으로 숨기면 낭독도 사라진다.

**기준** — 크기·줄바꿈이 적용되는가, 접근성 트리에 남는가로 나눈다.

**결론** — inline 은 크기 무시, none 은 낭독도 사라진다. 시각적 숨김은 clip 기법으로.

**비용** — visually-hidden 유틸 도입.

**측정** — 스크린리더로 숨긴 요소가 낭독되는지(visually-hidden)와 낭독되지 않는지(display:none)를 실제로 듣는다.
...
**비용** — 타이포·간격·테두리별 단위 규칙을 팀이 합의하고 기존 px 값을 rem 으로 옮기는 마이그레이션.

**측정** — 브라우저 글자 크기 200% 확대와 뷰포트 400px 에서 잘림·겹침이 없는지 시각 회귀로 확인한다.
...
**문제** — 새 배포가 즉시 반영되면서도 정적 자산은 최대한 캐시되어야 한다.

**대안** — 전부 no-cache 면 매번 다운로드, 전부 장기 캐시면 HTML 이 갱신되지 않는다.

**기준** — URL 이 바뀌는 자원인가(해시) 바뀌지 않는 자원인가(HTML)로 나눈다.

**결론** — 해시 자산은 1년 immutable, HTML 은 no-cache 매번 검증. 이 짝이 어긋나면 배포 직후 404 가 난다.

**비용** — 해시 파일명 빌드 파이프라인과 옛 청크 유지·청크 로드 실패 처리가 필요하다.

**측정** — 네트워크 패널 Size 열, 배포 직후 구버전 청크 404 발생률.
...
**문제** — 다른 오리진 API 를 브라우저에서 호출해야 한다.

**대안** — 프록시로 우회만 하면 조건을 모른 채 서버 비용이 붙고, 와일드카드 허용은 credentials 와 충돌.
...
**비용** — 허용 오리진·헤더를 서버에 정확히 열어야 하고, credentials 면 와일드카드가 불가라 오리진 목록을 관리해야 한다. preflight 왕복 하나가 남는다.

**측정** — OPTIONS 요청 수와 응답 헤더, 본 응답의 Allow-Origin 확인.
...
**문제** — 로그인 상태를 유지하면서 토큰 탈취 피해를 줄여야 한다.

**대안** — localStorage 는 편하지만 XSS 한 번이면 끝. 메모리만 쓰면 새로고침에 로그인이 풀린다.

**기준** — 어떤 공격(XSS vs CSRF)에 노출되는 저장소인가로 나눈다.

**결론** — 스크립트가 읽는 저장소는 XSS 한 번에 끝난다. HttpOnly 쿠키 + CSRF 대비, 또는 메모리 + 짧은 만료·회전이 기본이다.

**비용** — 쿠키는 CSRF 대비·다중 탭 갱신 잠금 코드가 필요하다.

**측정** — 저장 위치 인벤토리, 리프레시 재사용 감지 이벤트, 401 루프 발생률.
...
**문제** — 서버 변경을 사용자에게 실시간으로 전달해야 한다.

**대안** — 항상 WebSocket 은 인프라·재연결 설계 비용, 항상 폴링은 지연과 빈 응답.

**기준** — 방향(단방향/양방향)·빈도·운영 비용으로 나눈다.

**결론** — 단방향 알림은 SSE(또는 폴링), 양방향 저지연만 WebSocket. 어느 쪽이든 재연결 유실 보정이 본 설계다.

**비용** — SSE 는 단방향·HTTP/1.1 연결 한도, WS 는 프록시·스케일아웃·하트비트.

**측정** — 연결 수·재연결 빈도·유실 이벤트·도달 지연 분포.
...
**문제** — 크로스 사이트 상황에서 쿠키가 언제 붙는지 제어해야 한다.

**대안** — 값을 안 정하면 브라우저 기본(Lax)에 맡기게 되어 임베드가 조용히 깨진다.

**기준** — 크로스 사이트 요청에 쿠키가 붙는가로 나눈다.

**결론** — Strict 는 안 붙고, Lax 는 최상위 GET 에만, None 은 항상(Secure 필수). 임베드는 None 이 필요하다.

**비용** — None 은 Secure 강제, Strict 는 외부 링크 진입 시 로그아웃처럼 보인다.

**측정** — 임베드·외부 링크 진입 시나리오에서 쿠키 전송 여부를 네트워크 패널로.
...
**문제** — 요청 의미(안전·멱등)를 지켜 재시도·프리페치 사고를 막아야 한다.

**대안** — 편의상 상태 변경을 GET 링크로 두거나 전부 POST 로 두는 극단이 있다. 의미(안전·멱등)대로 나눠야 재시도·프리페치가 안전하다.

**기준** — 요청이 안전·멱등한가로 나눈다.

**결론** — GET 은 조회·멱등, POST 는 상태 변경·비멱등. 상태를 바꾸는 GET 은 프리페치·크롤러에 의해 실행된다.

**비용** — 상태를 바꾸는 GET 엔드포인트를 POST 로 바꾸는 API 재설계와 호출부 수정. 캐시·프리페치 정책도 따라 바뀐다.

**측정** — GET 으로 상태 변경하는 엔드포인트 스캔.
...
**비용** — 401 재인증·403 안내·429 백오프를 분기하는 인터셉터 코드와, 갱신 요청 자체를 재시도 대상에서 빼는 예외 처리.

**측정** — 401 루프 발생률, 429 재시도 성공률.
...
**문제** — 저장소 선택이 보안·수명 요구에 맞아야 한다.

**대안** — 편한 localStorage 일괄은 XSS 노출.
...
**비용** — IndexedDB 비동기 API 학습.

**측정** — 토큰·개인정보가 어느 저장소에 있는지 인벤토리를 만들고, HttpOnly 여부·수명·탭 간 공유를 각각 확인한다.
...
**문제** — 오프라인·업데이트를 동시에 안전하게 지원해야 한다.
...
**비용** — 활성화 정책·자원별 전략·갱신 UI.

**측정** — waiting 워커 비율, 캐시 오염 사고.
...
**문제** — 사용자 입력이 스크립트로 실행되지 않게 해야 한다.

**대안** — 입력 필터링만으론 컨텍스트별 인코딩을 못 대체하고, CSP 만으론 DOM 오염을 못 막는다.

**기준** — 방어의 본질이 입력 필터인가 출력 인코딩인가로 갈린다.

**결론** — 출력 컨텍스트별 인코딩이 본질이다. 위험 싱크를 막고 sanitizer 허용 목록, CSP 는 2차 방어.

**비용** — sanitizer 도입·허용 목록 유지, dangerouslySetInnerHTML 금지 린트.

**측정** — 자동 스캐너 + 마크다운·URL 입력 케이스 테스트.
...
**문제** — 인젝션이 성립해도 스크립트 실행을 2차로 막아야 한다.

**대안** — 한 번에 강제 모드로 켜면 무성 실패로 기능이 죽는다.
...
**비용** — 인라인 제거·nonce 발급·서드파티 협상, 리포트 수집 인프라.

**측정** — violation 리포트 수, report-only 기간의 위반 종류.
...
**문제** — 로그인 사용자의 권한으로 다른 사이트가 요청을 보내지 못하게 해야 한다.

**대안** — SameSite 만 믿으면 top-level POST·서브도메인 경로가 남는다.

**기준** — SameSite 가 막지 못하는 경로가 남아 있는가로 판단한다.

**결론** — 남는다(top-level POST·서브도메인·비쿠키 인증). 토큰 + Origin 검증을 유지하고 GET 으로 상태를 바꾸지 않는다.

**비용** — 토큰·Origin 검증 코드와 preflight 증가.

**측정** — 상태 변경 GET 스캔, Origin 불일치 요청 로그.
...
**문제** — 서드파티 코드가 우리 앱 권한으로 실행되는 위험을 통제해야 한다.

**대안** — '신뢰하는 라이브러리만 쓴다'는 절차가 아니다.

**기준** — 서드파티 코드가 어떤 권한으로 실행되는가로 본다.

**결론** — 같은 오리진 권한이다. CSP·SRI·iframe 격리와 승인 절차로 통제하고 lockfile·자동 PR·CI 로 의존성을 관리한다.

**비용** — CSP·SRI·격리 iframe·승인 절차·자동 업데이트 PR 운영 비용.

**측정** — 번들 증가 감시, 서드파티 스크립트 INP 영향, 의존성 감사 결과.
...
**문제** — 브라우저 앱이 토큰을 안전하게 얻고 유지해야 한다.

**대안** — implicit 은 토큰 URL 노출.

**기준** — 브라우저가 비밀을 지킬 수 있는가(없다)에서 출발한다.

**결론** — Authorization Code + PKCE, 짧은 액세스·회전 리프레시, 다중 탭 잠금. implicit 은 쓰지 않는다.

**비용** — PKCE·회전·다중 탭 잠금 구현.

**측정** — 토큰 URL 노출 0, 재사용 감지 이벤트.
...
**비용** — origin·스키마 검증 코드, sandbox 정책.
...
**문제** — 불필요한 렌더를 구분해 실제 문제만 고쳐야 한다.

**대안** — 모든 리렌더를 문제로 보고 memo 를 뿌리면 비교 비용만 늘어난다.

**기준** — 무엇이 렌더를 트리거하는가와 렌더가 곧 DOM 변경인가로 나눈다.

**결론** — state·부모 렌더·context·key 가 트리거다. 렌더≠커밋이라 리렌더 자체는 문제가 아니고 지표로 판단한다.

**비용** — 렌더/커밋 구분과 프로파일링 습관이 필요하다.

**측정** — React Profiler 렌더 사유, INP·프레임 지표.
...
**문제** — 목록이 바뀌어도 각 행의 상태가 올바른 데이터에 붙어 있어야 한다.

**대안** — 인덱스 key 는 정적 목록에선 무해하지만 정렬·삭제에서 깨진다. 랜덤 key 는 매번 재마운트.

**기준** — key 가 위치를 가리키나 데이터를 가리키나로 본다.

**결론** — 인덱스는 위치라서 삭제·정렬에 상태가 다른 행에 붙는다. 안정 id 를 쓰고, key 변경은 의도적 초기화 수단으로만.

**비용** — 안정 id 가 없으면 생성 시점에 부여하는 코드가 필요하다.

**측정** — 삭제·정렬 후 입력값 위치 e2e.
...
**문제** — 비동기 결과가 최신 상태만 반영하고 정리가 누락되지 않아야 한다.

**대안** — 이펙트 의존성을 비우고 eslint-disable 하면 stale, 매 컴포넌트에 취소 코드를 반복하면 누락.

**기준** — 이펙트가 잡은 값이 최신인가, 이펙트가 남긴 것이 정리되는가로 나눈다.

**결론** — 의존성을 비우면 stale, 클린업이 없으면 누수·경합. 구독·타이머·요청은 클린업하고, 파생 값은 이펙트가 아니다.

**비용** — 데이터 페칭 계층(쿼리 라이브러리) 도입 비용.

**측정** — 응답 순서 뒤집기 재현, AbortError 카운트, StrictMode 이중 실행.
...
**문제** — 렌더 비용을 줄이되 코드 복잡도와 버그를 늘리지 않아야 한다.

**대안** — 전부 감싸기(비용·stale 위험) vs 전부 안 쓰기(memo 자식 무효화).

**기준** — 참조 동일성이 소비자에게 의미가 있는가와 계산이 실제로 비싼가로 나눈다.

**결론** — 둘 중 하나가 참일 때만 쓴다. 기본은 안 쓰고 프로파일러로 측정한 뒤 넣는다.

**비용** — 의존성 관리 부담, 컴파일러 도입 시 재판단.

**측정** — Profiler 로 memo 효과 확인, 제거 전후 렌더 시간.
...
**문제** — 전역 상태 변경이 필요한 곳만 리렌더되어야 한다.

**대안** — Context 분할만으로는 부분 구독이 안 되고, 스토어 도입은 의존성이 늘어난다.

**기준** — Context 가 부분 구독을 지원하는가(못 한다)로 출발한다.

**결론** — value 메모 → Context 분할 → 부분 구독이 필요하면 selector 스토어. 새 참조 selector 는 다시 같은 문제를 만든다.

**비용** — 스토어·selector 규율(얕은 비교) 학습 비용.

**측정** — Profiler 리렌더 범위, value 참조 변경 로그.
...
**문제** — 서버 데이터의 신선도·일관성을 한 곳에서 관리해야 한다.

**대안** — 전역 스토어에 복사하면 동기화 코드가 늘어나고 두 진실이 생긴다.

**기준** — 데이터의 원본이 어디 있는가(서버 사본 vs 화면 소유)로 나눈다.

**결론** — 서버 사본은 캐시 계층, UI 상태는 로컬, 공유·복원은 URL. 사본을 스토어에 복사하면 진실이 둘이 된다.

**비용** — 캐시 라이브러리 학습·키 설계 비용.

**측정** — 같은 엔티티 저장 위치 수, 수정 후 다른 화면 값 e2e.
...
**문제** — 상태를 잘못 둬서 생기는 동기화·복원 문제를 피해야 한다.

**대안** — 습관적 전역은 결합, 습관적 로컬은 공유·복원 불가.

**기준** — 누가 읽고 얼마나 오래 살아야 하는가로 나눈다.

**결론** — 가장 좁은 범위에 두고 필요할 때만 올린다. 공유·복원은 URL, 서버 데이터는 캐시, 편의값은 로컬 스토리지.

**비용** — URL 상태는 히스토리 오염·길이 제한, 서버 저장은 왕복.

**측정** — 새로고침·공유·뒤로 가기 시 상태 복원 e2e.
...
**문제** — 서버 HTML 과 클라이언트 렌더가 일치해야 한다.

**대안** — 경고 억제는 트리 재생성과 깜빡임을 숨길 뿐.

**기준** — 서버와 클라이언트가 같은 값을 만들 수 있는가로 나눈다.

**결론** — 못 만드는 값(시간·랜덤·window)은 마운트 후 렌더로 분리한다. 경고 억제는 답이 아니다.

**비용** — 클라이언트 전용 분리·Suspense 경계 설계 비용.

**측정** — 하이드레이션 경고 0, 전체 트리 재생성 여부 프로파일.
...
**문제** — 이펙트가 마운트·언마운트를 반복해도 안전해야 한다.

**대안** — StrictMode 를 끄면 결함이 프로덕션에서 드러난다.
...
**측정** — 개발 모드 이중 실행에서 중복 전송·구독 수 확인.
...
**문제** — 렌더에 반영되지 않아야 하는 값과 반영되어야 하는 값을 분리해야 한다.

**대안** — 리렌더 회피용으로 ref 에 상태를 넣으면 UI 불일치.

**기준** — 그 값이 화면에 반영되어야 하는가로 나눈다.

**결론** — 반영되면 state, 아니면(포커스·측정·인스턴스) ref. 리렌더를 피하려고 상태를 ref 에 넣지 않는다.

**비용** — 명령형 핸들 노출은 상태 소유권 문제를 가릴 수 있다.

**측정** — ref 변경 후 UI 갱신 누락 테스트, 명령형 경로 수 추적.
...
**문제** — 폼 제출·낙관적 표시·에러를 수동 상태 없이 다뤄야 한다.

**대안** — 수동 isSubmitting·useState 조합은 경합·롤백 누락이 생긴다.
...
**비용** — React 19 API 학습과 서버 검증 재설계.

**측정** — 실패 후 낙관적 값 복귀 e2e, 중복 제출 방지.
...
**문제** — 번들 크기와 워터폴을 줄이되 상호작용은 유지해야 한다.

**대안** — 전부 클라이언트(번들 큼) vs 전부 서버(상호작용 불가).

**기준** — 이 컴포넌트에 상태·이벤트·브라우저 API 가 필요한가로 나눈다.

**결론** — 기본은 서버, 필요한 잎만 클라이언트. 경계를 넘는 props 는 직렬화 가능해야 하고, 경계 자리가 적으면 도입 이득이 없다.

**비용** — 경계 설계·직렬화 제약·운영 역량.

**측정** — 클라이언트 번들 크기, 경계 수, 첫 화면 지표 전후.
...
**대안** — props 를 자식이 바꾸면 어긋남.

**기준** — 값을 누가 소유하고 누가 바꾸는가로 나눈다.

**결론** — props 는 부모 소유·읽기 전용, state 는 자기 소유. 같은 값을 두 곳이 갖지 않게 한 소유자를 정한다.

**비용** — 상태를 공통 부모로 올리는 리프팅 코드와 콜백 전달이 늘어난다. 부모가 너무 멀면 컨텍스트·스토어 검토가 따라온다.

**측정** — 같은 값을 두 곳 이상에서 state 로 갖고 있는 중복 상태 수, 수정 후 어긋나는 화면이 없는지.
...
**문제** — 렌더 모델을 정확히 이해해 성능 오해를 피해야 한다.

**대안** — '가상 DOM 이 빠르다' 오해는 잘못된 최적화.

**기준** — 빠르기 위한 것인가 선언적 모델을 위한 것인가로 본다.

**결론** — 선언적 모델과 최소 커밋이 요점이다. 수작업 DOM 보다 빠르다는 보장은 없다.

**비용** — 재조정 규칙(key·타입 동일성)을 이해하고 렌더마다 새 컴포넌트 타입을 만드는 패턴을 걷어내는 학습·리팩터.
...
**문제** — 폼 크기·검증 요구에 맞는 입력 제어 방식을 택해야 한다.

**대안** — 항상 제어는 큰 폼에서 렌더 폭발.
...
**측정** — 키 입력 한 번당 리렌더되는 컴포넌트 수(Profiler), 큰 폼에서 입력 지연(INP).
...
**문제** — 조건부 렌더에서 의도치 않은 값이 화면에 안 나와야 한다.
...
**비용** — 조건부 렌더에 명시 비교(count > 0)를 쓰는 팀 규칙과 린트. 기존 && 패턴을 찾아 고치는 작업.

**측정** — count 가 0·NaN 인 케이스의 렌더 결과 테스트, 화면에 '0' 이 찍히는 회귀가 없는지.
...
**문제** — 긴 렌더가 입력을 막지 않고 로딩이 UI 구조에 맞아야 한다.
...
**비용** — Suspense 경계 위치를 UI 구조에 맞게 설계하고 긴급/비긴급 업데이트를 구분하는 코드. 경계를 잘못 두면 깜빡임이 생긴다.

**측정** — INP, 경계 fallback 빈도.
...
**문제** — 즉시 반응하는 UI 와 서버 진실을 동시에 지켜야 한다.

**대안** — 확정 후 반영은 느리고, 롤백 없는 낙관은 거짓 상태.
...
**측정** — 실패 주입 e2e, 연속 클릭 최종 상태, 서버 중복 생성 수.
...
**문제** — 변경 후 화면들이 일관되면서 리페치는 최소여야 한다.

**대안** — 전량 무효화는 단순하지만 폭증, 무효화 누락은 옛 값.

**기준** — 캐시 키가 부분 무효화를 허용하는 구조인가로 출발한다.

**결론** — 엔티티+파라미터 계층 키를 두고, 응답으로 직접 갱신(write-through)하고 필요한 범위만 무효화한다. 전량 무효화는 비용이다.

**비용** — 키 계층 설계·write-through 코드.

**측정** — 수정 1회당 요청 수, 히트율, 일관성 e2e.
...
**문제** — 대량 목록에서 항목 수정 후 위치·일관성을 유지해야 한다.

**대안** — 전체 리페치는 스크롤 초기화, 오프셋은 중복·누락.
...
**문제** — 동시 편집·오프라인에서 데이터가 조용히 사라지지 않아야 한다.
...
**문제** — 외부 입력을 검사 없이 쓰는 코드를 컴파일 단계에서 막아야 한다.

**대안** — any 는 편하지만 전파되어 검사가 사라진다.
...
**문제** — 타입 불일치를 숨기지 않고 근본 해결해야 한다.

**대안** — as 는 빠르지만 회귀를 침묵시킨다.

**기준** — 단언이 컴파일러가 잡을 회귀를 숨기는가로 본다.

**결론** — 숨긴다. 가드·유니온·제네릭·스키마·satisfies 로 대체하고, as const 만 예외다.

**비용** — 가드·스키마·satisfies 로 코드가 늘어난다.

**측정** — as 사용 수·리뷰 차단, 런타임 파싱 실패율.
...
**문제** — 의미가 다른 같은 모양의 값이 섞이지 않아야 한다.

**대안** — 구조적 타이핑 그대로 두면 id 혼동 컴파일 통과.

**기준** — 모양이 같으면 다른 의미의 값이 섞이는가로 본다.

**결론** — 섞인다. 식별자·단위는 branded 타입으로 분리하고 변환 함수만 통과시킨다.

**비용** — branded 타입 변환 함수·도입 범위 한정.

**측정** — 혼동 사고 재발 0, 컴파일 에러로 조기 검출.
...
**문제** — 외부 데이터가 타입과 다를 때 깊은 곳에서 터지지 않게 해야 한다.

**대안** — 내부 곳곳 방어 코드는 원인 추적 불가, 검증 생략은 회귀.

**기준** — 신뢰 경계가 어디인가(경계 밖 입력은 전부 unknown)로 나눈다.

**결론** — 경계에서 한 번 파싱하고 내부는 파싱된 타입만 신뢰한다. 성능 이유로 검증을 빼면 누락 회귀가 온다.

**비용** — 스키마 정의·파싱 실패 처리·성능 점검.

**측정** — 파싱 실패 로그, 계약 CI 대조.
...
**문제** — 첫 콘텐츠 표시가 늦는 원인을 특정해야 한다.

**대안** — 이미지 압축만 하면 TTFB·렌더 지연 원인일 때 무효.

**기준** — LCP 를 네 구간(TTFB·로드 지연·로드 시간·렌더 지연)으로 쪼갠다.

**결론** — 지배 구간부터 고친다. 히어로 이미지 우선순위·지연 로딩 해제·렌더 블로킹 축소가 순서다.

**비용** — preload 남용은 대역폭 경쟁.

**측정** — LCP 네 구간 attribution, 워터폴 시작 순번.
...
**문제** — 번들 증가가 사용자 체감으로 이어지지 않게 해야 한다.

**대안** — 무작정 lazy import 는 요청 폭발.
...
**비용** — RUM·세그먼트·예산·CI 인프라.

**측정** — p75/p95 전후, 세그먼트 악화 여부.
...
**문제** — 이미지가 첫 화면과 CLS 를 망치지 않아야 한다.

**대안** — 전부 lazy 는 LCP 악화, 전부 eager 는 대역폭 낭비.

**기준** — 첫 화면(LCP) 이미지인가 화면 밖 이미지인가로 나눈다.

**결론** — LCP 이미지는 eager+우선순위, 나머지는 lazy. srcset·최신 포맷·크기 지정으로 CLS 를 막는다.

**비용** — srcset·포맷 파이프라인 운영.

**측정** — LCP 이미지 시작 시각, CLS attribution.
...
**문제** — 브랜드 폰트를 쓰면서 깜빡임·이동을 억제해야 한다.

**대안** — 시스템 폰트만 쓰면 브랜드 요구 미충족, 무조건 swap 은 CLS.
...
**측정** — CLS attribution 의 텍스트 요소, 폰트 요청 시각.
...
**문제** — 첫 방문과 이후 이동 속도를 함께 만족해야 한다.

**대안** — CSR 고정은 첫 화면, MPA 고정은 이동.

**기준** — 첫 방문 지표와 이후 이동 속도 중 무엇이 중요한가로 나눈다.

**결론** — CSR 은 JS 뒤에야 데이터가 시작돼 첫 화면이 늦다. 서버 렌더·프리페치로 첫 화면을, SPA 이동은 그대로 살린다.

**비용** — 서버 렌더를 도입하면 서버 인프라·캐시 무효화·하이드레이션 비용과 관측 대상이 늘어난다.
...
**비용** — attribution 계측·전환 설계.

**측정** — web-vitals attribution 으로 입력 지연·처리·표현 세 구간의 p75 분포와 어떤 상호작용(target)이 지배하는지.
...
**문제** — 대용량 파일을 실패해도 이어 올릴 수 있어야 한다.
...
**문제** — 프로토콜 변화에 맞게 번들을 나눠야 한다.

**대안** — 옛 규칙(스프라이트·과도 인라인)이나 과분할 둘 다 손해.
...
**문제** — 테스트 비용 대비 실제 결함을 잡아야 한다.

**대안** — 커버리지 목표는 가짜 안심, 전면 e2e 는 느리고 불안정.
...
**문제** — 테스트가 리팩터에 견디고 접근성을 드러내야 한다.

**대안** — testid 우선은 구현 결합, 클래스 선택자는 즉시 깨짐.

**기준** — 사용자가 화면을 인식하는 축으로 찾는가로 본다.

**결론** — role→label→text→testid 순. 접근성 트리 기준이라 구현 변경에 견디고 접근성 결함도 드러난다.

**비용** — role/label 부재 컴포넌트는 먼저 고쳐야 한다.

**측정** — 리팩터 후 테스트 변경 수, 접근성 결함 검출.
...
**문제** — 테스트가 실제 결함을 잡고 리팩터에 부서지지 않아야 한다.

**대안** — 내부 모듈 모킹은 가짜 green.

**기준** — 모킹 경계가 네트워크인가 내부 모듈인가로 나눈다.

**결론** — 네트워크 경계만 모킹한다. 내부 모듈 모킹은 리팩터에 부서지고 가짜 통과를 만든다.

**비용** — MSW 등 경계 모킹 설정, 계약 대조.

**측정** — 리팩터 후 깨진 테스트 수, 계약 불일치 검출.
...
**문제** — 게이트가 결함을 막되 배포를 멈추지 않아야 한다.

**대안** — 모든 테스트를 차단 조건으로 걸면 배포가 멈추고 우회 문화가 생긴다. 반대로 게이트가 없으면 결함이 그대로 나간다.

**기준** — 실패가 사용자 피해로 이어지는가(차단) 아닌가(경고)로 나눈다.

**결론** — 핵심 흐름 e2e·타입·보안은 차단, 커버리지·예산 근접은 경고. 불안정 테스트는 차단에서 빼고 고친다.

**비용** — 차단/경고 분류·불안정 테스트 관리.
...
**문제** — 안 쓰는 코드가 번들에 남지 않아야 한다.

**대안** — '번들러가 알아서'는 CJS·배럴에서 실패.

**기준** — 번들러가 정적으로 사용 여부를 알 수 있는 구조인가로 본다.

**결론** — ESM·sideEffects 표기·배럴 최소화·프로덕션 모드. 확인은 산출물 분석으로 한다.

**비용** — ESM·sideEffects·배럴 정리 작업.

**측정** — 산출물 분석에서 모듈 단위 포함 여부.
...
**문제** — 두 모듈 형식 혼용에서 런타임 오류가 없어야 한다.
...
**비용** — exports 조건·번들러 설정 이해.

**측정** — 해석된 파일 확인, 싱글턴 이중 여부.
...
**비용** — 린트 규칙·예외 승인 절차·CI 시간.

**측정** — 순환 의존 0, 영향 그래프 빌드 시간.
...
**문제** — 프로덕션 에러를 원인 코드로 되돌릴 수 있어야 한다.

**대안** — 콘솔·사용자 제보만으론 재현 불가.

**기준** — 난독화된 스택을 원본으로 되돌릴 수 있는가로 본다.

**결론** — 소스맵 업로드(비공개) + 릴리스 태깅 + 전역 핸들러. 어느 배포에서 시작됐는지가 갈려야 한다.

**비용** — 소스맵 업로드·릴리스 태깅 파이프라인.

**측정** — 복원된 스택 비율, 버전별 에러율.
...
**문제** — 의존성 설치가 재현 가능하고 배포 산출물이 가벼워야 한다.

**대안** — lockfile 미커밋은 재현 불가.

**기준** — 런타임 필요 여부와 재현성으로 나눈다.

**결론** — 런타임 필요만 dependencies, 도구는 devDependencies. lockfile 은 재현성이라 커밋하고 손으로 고치지 않는다.

**비용** — 의존성 자동 갱신 PR 을 검토·병합하는 운영 시간과 lockfile 충돌 시 재생성 규칙.

**측정** — clean install 시간, 프로덕션 번들·이미지 크기, lockfile 불일치로 생긴 환경 차이 사고 수.
...
**문제** — 지원 범위와 폴리필 비용을 데이터로 정해야 한다.

**대안** — 모든 브라우저 지원은 전원에게 레거시 번들.
...
**문제** — 플래그가 늘어도 조합 폭발·관측 불가가 되지 않아야 한다.

**대안** — 플래그 많이 두면 안전이라는 오해.

**기준** — 플래그의 종류와 수명을 구분하는가로 본다.

**결론** — 릴리스·실험·킬스위치·권한을 나누고 단기 토글엔 제거 기한과 담당자를 붙인다. 첫 페인트 전에 값을 확정한다.

**비용** — 종류·수명·제거 기한 관리, 플리커 방지.

**측정** — 활성 플래그 수, 제거 이행률, CLS.
...
**문제** — 누구나 키보드·보조기기로 조작할 수 있어야 한다.

**대안** — div 에 role/tabindex/키 핸들러 전부 붙이는 건 비용.

**기준** — 기본 동작을 브라우저가 주는가, 내가 전부 다시 만드는가로 나눈다.

**결론** — button 은 포커스·키보드·role·disabled 를 공짜로 준다. div 면 그 전부를 직접 붙여야 하니 태그 교체가 가장 싸다.

**비용** — 디자인이 기본 스타일을 거부하면 초기화 CSS.

**측정** — 키보드만으로 조작 e2e, Accessibility 트리.
...
**문제** — 모달이 포커스·배경을 올바르게 다뤄야 한다.

**대안** — 오버레이 div 만 두면 Tab 이 배경으로 샌다.

**기준** — 포커스가 어디로 가고 어디로 돌아오는가, 배경이 정말 비활성인가로 본다.

**결론** — 네이티브 dialog.showModal 이 대부분을 처리한다. 직접 만들면 이동·트랩·복귀·Esc·inert 를 다 챙긴다.

**비용** — 네이티브 dialog 지원 범위, 중첩 스택 관리.

**측정** — 키보드 열기→조작→닫기 e2e, activeElement 복귀.
...
**문제** — ARIA 가 접근성을 깨뜨리지 않아야 한다.

**대안** — 'ARIA 많이 붙이면 좋다'는 트리 파괴.

**기준** — 네이티브가 이미 제공하는 의미를 ARIA 가 덮어쓰고 있는가로 본다.

**결론** — ARIA 첫 규칙: 가능하면 쓰지 않는다. 잘못된 role·라벨 불일치·포커스 가능한 aria-hidden 이 대표 사고다.

**비용** — 이미 붙인 ARIA 를 걷어내고 네이티브 요소로 되돌리는 리팩터. 커스텀 위젯은 APG 패턴을 정확히 따르는 학습이 필요하다.

**측정** — computed role/name 대조, aria 규칙 자동 검사.
...
**문제** — 이미지의 의미가 보조기기에 정확히 전달되어야 한다.

**대안** — 전부 채우기/전부 비우기는 둘 다 틀림.

**기준** — 이미지가 정보를 전달하는가, 장식인가, 컨트롤의 유일한 이름인가로 나눈다.

**결론** — 정보면 문장, 장식이면 alt="", 컨트롤이면 동작 이름. 속성 누락은 파일명 낭독이다.

**비용** — 이미지마다 정보/장식을 판단해 alt 를 쓰는 콘텐츠 규칙과 디자이너·에디터 교육. CMS 라면 필드 필수화.

**측정** — computed name, 자동 검사.
...
**문제** — 보조기기 사용자가 페이지 구조(제목·영역·목록)를 건너뛰며 탐색할 수 있어야 하고, 폼·버튼의 기본 동작이 공짜로 따라와야 한다.
...
**비용** — div 로 짜던 습관을 시맨틱 요소로 바꾸는 학습과 기존 마크업 정리. 스타일 초기화 CSS 가 따라온다.
...
**문제** — 구·신 버전 공존 중에도 깨지지 않게 변경해야 한다.

**대안** — 한 번에 바꾸면 블루그린에서 반드시 깨진다.

**기준** — 구·신 버전이 동시에 살아 있는 구간이 있는가로 출발한다.

**결론** — 블루그린이면 반드시 있다. Expand→Migrate→Contract 로 나누고 각 단계가 단독 롤백 가능해야 한다.

**비용** — 단계별 코드·플래그·정리 기한 관리.

**측정** — 단계별 롤백 리허설, 옛 형태 사용량 0 확인.
...
**문제** — 변경이 한 곳에 머물고 재사용이 잘못된 추상을 만들지 않아야 한다.
...
**문제** — 장애를 사용자 제보 전에 알고 원인을 좁혀야 한다.

**대안** — 성공 경로만 계측하면 안 보인다.
...
**문제** — 실시간 스트림 UI 가 성능·유실·비용을 동시에 지켜야 한다.

**대안** — 토큰마다 setState 는 INP 붕괴.
...
**측정** — 제안 전과 후의 리드타임·결함 재발률·번들 크기 같은 같은 지표를 비교해 보고한다. 수치가 없으면 설득이 아니라 호소다.
...
**문제** — 리뷰가 실질 결함을 막고 병목이 되지 않아야 한다.

**대안** — 전부 동일 무게면 LGTM 문화.
...
**문제** — 원인을 확정해 재발을 막아야 한다.

**대안** — 증상만 보고 추측으로 고치면 같은 버그가 다른 자리에서 재발하고, 재현 없이 로그만 늘리는 것도 원인 확정이 아니다.

**기준** — 원인을 추정으로 말하나 측정으로 확정했나로 본다.

**결론** — 재현 → 가설 하나씩 측정 → 범인 지점 → 근본 수정 → 재발 방지 테스트. 확정 증거를 말할 수 있어야 한다.

**비용** — 재현 환경을 만들고 가설을 하나씩 측정하는 시간, 재발 방지 테스트를 유지하는 비용.
...
**비용** — 대안마다 구현 기간·성능·접근성 영향을 산출하고 결정과 근거를 기록하는 시간. 관철되지 않아도 기록은 남긴다.
...
**문제** — 화면마다 맞는 렌더링 방식을 골라 첫 화면·운영 비용을 함께 맞춰야 한다.

**대안** — 프레임워크 기본값 일괄 적용은 화면별 요구를 무시한다.

**기준** — 콘텐츠가 공개·정적인가 사용자별·실시간인가, 첫 화면 지표가 사업에 중요한가로 나눈다.

**결론** — 정적이면 사전 렌더, 개인화면 서버 렌더 또는 클라이언트 페치. 화면 단위로 섞고 운영 비용을 함께 계산한다.

**비용** — 전략 혼용 시 캐시·에러 경계 복잡도, 서버 운영.

**측정** — 화면별 LCP·TTFB, 서버 비용.
...
**문제** — 레거시를 서비스 중단 없이 새 구조로 옮겨야 한다.
...
**비용** — 이중 유지·어댑터·이관 지표 관리.

**측정** — 트래픽 이관 비율, 에러율, 제거 완료 시점.
...
**문제** — 결정 이유가 사람과 함께 사라지지 않아야 한다.

**대안** — 위키 서술은 검색·폐기 조건이 없다.

**기준** — 결정의 맥락·대안·폐기 조건이 남는가로 본다.

**결론** — ADR 로 남기고 되돌릴 때는 새 ADR 로 supersede. 기록을 지우지 않는다.

**비용** — 결정마다 ADR 을 쓰고 재검토 시점을 지키는 규율. 초기엔 속도가 느려진 것처럼 느껴진다.
...
**문제** — 여러 앱이 한 시스템을 쓰면서 각자 깨지지 않아야 한다.

**대안** — 컴포넌트 수 늘리기는 채택과 무관.
...
**비용** — 롤백 가능한 배포 설계, 사후 분석 시간.

**측정** — 감지→복구 시간, 재발 항목 이행.
...
**문제** — 규모가 커져도 지식·책임이 흐려지지 않아야 한다.

**대안** — 문서만 더 쓰는 건 실행되지 않는다.

**기준** — 지식이 사람에 고이는가 구조에 남는가로 본다.

**결론** — CODEOWNERS 로 소유를 명시하고 ADR·순환 리뷰로 분산한다. 온보딩 지표는 첫 배포까지 걸린 시간.

**비용** — CODEOWNERS·ADR·자동화 도구 운영.

**측정** — 첫 배포까지 시간, 무소유 코드 비율.
...
**문제** — 이벤트 폭주를 줄이되 마지막 입력을 잃지 않아야 한다.

**대안** — 매 이벤트 처리는 낭비, 단순 타이머는 마지막 유실.
...
**문제** — 대량 요소 클릭을 리스너 폭발 없이 처리해야 한다.
...
**문제** — 끝없는 목록을 중복·누락 없이 불러야 한다.

**대안** — 스크롤 이벤트 계산·오프셋은 부정확.

**기준** — 페이징 방식(커서 vs 오프셋)과 중복 로드 차단이 있는가로 본다.

**결론** — IntersectionObserver 센티넬 + 커서 페이징 + 진행 플래그. 수백 행 넘으면 가상화, 키보드 폴백은 남긴다.

**비용** — 커서 페이징 서버 계약, 센티넬·IntersectionObserver·진행 플래그 구현, 키보드 사용자용 '더 보기' 폴백 유지.
...
**문제** — 슬라이드 UI 가 터치·키보드·모션 설정을 만족해야 한다.

**대안** — transform 계산 전부 JS 는 비용.

**기준** — CSS 가 할 수 있는 것과 JS 가 필요한 것을 나눈다.

**결론** — 레이아웃·스냅은 CSS, 인덱스 동기화만 JS. 자동 재생은 정지 수단과 모션 축소 존중이 필수다.

**비용** — 자동 재생이면 정지 버튼·포커스 시 정지·모션 축소 존중을 구현해야 하고, 무한 순환 복제 슬라이드의 접근성 처리.
...
**문제** — 입력 오류를 사용자가 고칠 수 있게 알려야 한다.

**대안** — 정규식만 직접 짜면 접근성·서버 검증 누락.
...
**문제** — 실패 요청을 안전하게 재시도하고 동시성을 제한해야 한다.

**대안** — 고정 지연 무한 재시도는 폭주·중복.

**기준** — 재시도해도 되는 에러인가와 동시 실행 상한이 있는가로 본다.

**결론** — 5xx·429·네트워크만 백오프+지터로 재시도, 비멱등은 요청 키. pLimit 은 실패에도 카운터를 되돌린다.

**비용** — 재시도 가능 에러 판별, 지터 백오프, 멱등 요청 키, 큐 카운터 관리 코드. 서버가 멱등을 보장해야 안전하다.
...
**문제** — 거대 목록을 라이브러리 없이 끊김 없이 그려야 한다.

**대안** — 전량 렌더는 불가, CSS 대안 검토 없이 구현은 과함.

**기준** — 보이는 범위만 그릴 때 잃는 것을 감수하는가로 먼저 판단한다.

**결론** — 인덱스 계산으로 창만 렌더하고 스페이서로 스크롤바를 유지한다. 그 전에 content-visibility 가 되는지 본다.

**비용** — 가변 높이면 측정 캐시·보정 로직이 필요하고, 브라우저 검색·인쇄·스크린리더 탐색을 잃는다.
...
**문제** — 순서 변경을 모든 입력 장치에서 저장까지 안전하게 해야 한다.

**대안** — 마우스 이벤트만은 터치·키보드 누락.

**기준** — 입력 장치를 한 경로로 다루나, 키보드 대안이 있나로 본다.

**결론** — 포인터 이벤트 + 캡처, 시작 시 1회 측정, 낙관적 반영 후 실패 롤백. Alt+방향키 대안은 필수다.

**비용** — 간격을 둔 정렬키 모델(서버 계약), 낙관적 반영 후 실패 롤백, 키보드 대안 구현.

**측정** — 키보드 대안 e2e, 저장 실패 롤백.
...
**문제** — 외부 스토어를 React 와 찢어짐 없이 연결해야 한다.

**대안** — useState+전역 변수는 tearing·누수.

**기준** — 동시성 렌더에서 찢어짐(tearing)을 막을 수 있는가로 본다.

**결론** — useSyncExternalStore 로 연결하고 selector 는 원시값·얕은 비교. SSR 스냅샷을 따로 준다.

**비용** — selector 규율·SSR 스냅샷.

**측정** — 리렌더 루프 0, 구독 해제 확인.
...
**문제** — SPA 전환이 히스토리·접근성을 지켜야 한다.

**대안** — hashchange·pushState 만은 포커스·예외 누락.

**기준** — 가로채지 말아야 할 링크를 구분하고 전환 후 접근성을 챙기는가로 본다.

**결론** — pushState/popstate 에 외부·새 탭·수정키 예외, 전환 후 제목·포커스·live region·스크롤 복원.

**비용** — 외부 링크·새 탭·수정키 예외 처리, 전환 후 포커스·제목·스크롤 복원 코드를 직접 유지해야 한다.

**측정** — 뒤로 가기 e2e, 스크린리더 안내.
...
**문제** — 이미지 지연 로딩이 LCP 를 해치지 않아야 한다.
...
**비용** — IntersectionObserver 를 직접 쓰면 로드 후 unobserve 와 자리 확보(aspect-ratio)를 빠뜨리지 않아야 한다.

**측정** — LCP 이미지 시작 시각, CLS.
...
**문제** — 표 정렬·필터가 정확하고 접근 가능해야 한다.

**대안** — 기본 sort 는 한글·숫자 틀림.
...
**문제** — 설계 라운드에서 트레이드오프를 시간 안에 보여야 한다.
...
**비용** — 요구 탐색에 5분 이상을 쓰는 결정. 짧은 라운드에서는 최적화 세부를 포기해야 한다.

**측정** — 면접관 평가 축(질문 수·트레이드오프 문장).
...
**문제** — 피드가 갱신되어도 위치·일관성이 유지되어야 한다.

**대안** — 서버 API 나열은 갱신 처리 누락.
...
**문제** — 스트리밍 채팅이 성능·비용·보안을 지켜야 한다.

**대안** — innerHTML 직접 삽입은 XSS.
...
**측정** — INP, 취소 후 서버 비용, sanitize 테스트.
...
**비용** — 메모리·HTTP·엣지 세 계층의 캐시 키와 무효화를 운영하고, 지표(첫 결과 지연·히트율)를 수집하는 인프라.
...
**문제** — 협업 편집이 수렴·유실 없이 입력 지연을 지켜야 한다.

**대안** — 'CRDT 쓰면 된다'는 재연결·권한 누락.
...
**문제** — GB 단위 파일이 중간에 실패해도 처음부터 다시 올리지 않아야 하고, 업로드 중 화면이 굳지 않아야 한다.

**대안** — FormData 단발은 실패 시 처음부터.

**기준** — 재개·멱등·동시성 조절과 접근 가능한 선택 경로가 있는가로 본다.

**결론** — 청크+병렬+세션 재개+워커 해시. 완료는 서버 검증까지, 파일 선택 버튼은 남긴다.

**비용** — 청크 크기·세션 만료·멱등 키·완료 검증을 서버와 합의하는 계약 비용, 해시·압축을 워커로 보내는 구현.
...
**문제** — 위젯 하나가 느리거나 죽어도 나머지 대시보드는 보이고, 위젯 20개가 동시에 요청을 쏟지 않아야 한다.
...
**문제** — 결제가 중복·재진입에 안전해야 한다.

**대안** — 제출 버튼에 로딩만 두면 새로고침·뒤로 가기·타임아웃에서 중복 결제가 난다. 낙관적 업데이트로 '완료'를 먼저 보이는 것은 금융에서 금지.
...
**문제** — 컴포넌트 라이브러리가 파괴적 변경에도 소비자를 깨지 않아야 한다.

**대안** — 목록·스토리북만은 전파 전략 부재.

**기준** — 공개 API 경계가 있는가와 파괴적 변경을 어떻게 전파하는가로 나눈다.

**결론** — props·슬롯·토큰만 공개하고 ESM·세맨틱 버전·코드모드로 배포한다. 접근성은 계약에 넣는다.

**비용** — API 경계·배포 포맷·코드모드.

**측정** — 앱별 최신 버전 채택률, 내부 DOM 을 CSS 로 덮어쓴 오버라이드 수, 접근성 회귀 검출 수.
...
**문제** — 네트워크가 없어도 노트를 쓰고, 복귀했을 때 두 기기의 변경이 조용히 덮어써지지 않아야 한다.

**대안** — localStorage 덮어쓰기는 손실.

**기준** — 원본이 어디인가(로컬 우선)와 충돌을 어떻게 다루는가로 나눈다.

**결론** — IndexedDB 가 원본, 서버 동기화는 큐 + 충돌 감지. 마지막 쓰기 승리는 손실이다. 저장소 한계에 대비한다.

**비용** — IndexedDB·큐·충돌 규칙.

**측정** — 두 기기 동시 편집 시나리오의 충돌 발생·해결 수, 사용자 손실 신고, 저장소 한계 도달 빈도.
...
**문제** — 언어를 추가할 때마다 문장 조립·날짜·통화·정렬 코드를 고치지 않아야 하고, RTL 과 긴 번역에서 레이아웃이 살아야 한다.

**대안** — 키-값 치환만은 복수형·정렬 실패.
...
**문제** — 프리즈 원인을 측정으로 찾아야 한다.

**대안** — 코드 읽기·메모이제이션 선행은 오진.

**기준** — 추측으로 코드를 읽나 측정으로 구간을 좁히나로 출발한다.

**결론** — Performance 녹화 → Long Task 스택 → 핸들러/렌더/레이아웃 구간 분해. 처방은 원인별로 다르다.

**비용** — 코드를 읽기 전에 프로파일러를 여는 습관과 performance.mark 를 심는 계측 코드. 재현 조건(저사양·스로틀링)을 갖춰야 한다.

**측정** — Long Task 스택, 구간 분해.
...
**문제** — API 는 200 을 돌려주는데 목록이 가끔 비어 보인다. 재현이 열 번에 한 번이고 서버 로그엔 이상이 없다.

**대안** — 서버 팀에 넘기거나 새로고침으로 넘어가는 건 원인 미해결이다. 화면 전체를 감싼 try/catch 로 덮는 것도 증상 은폐다.

**기준** — 네트워크는 성공했으니 클라이언트 처리 어디가 실패했나로 좁힌다.

**결론** — 응답 형태·경합·삼켜진 예외·캐시 오염 순으로 본다. Pause on exceptions 와 지연 주입으로 재현한다.

**비용** — 네트워크 응답에서 렌더까지의 경로에 로그를 심고, 경합을 강제로 재현하는 지연 주입 코드를 유지해야 한다.

**측정** — 응답 도착·파싱·렌더 분기 로그, 'Pause on exceptions' 로 삼켜진 예외, 지연 주입 후 재현 성공률.
...
**문제** — 콘솔에 'Too many re-renders' 가 뜨고 화면이 멈춘다. 어느 상태가 매 렌더 바뀌는지 모른다.

**대안** — useMemo/useCallback 을 전부 감싸면 우연히 멈출 수 있지만 원인은 남는다. 의존성 배열을 비우고 린트를 끄면 stale 버그로 바뀐다.

**기준** — 무엇이 매 렌더 바뀌는지 로그로 확정했는가로 본다.

**결론** — 렌더 중 setState·새 참조 의존성·새 참조 selector 가 후보. 메모이제이션 전에 데이터 흐름을 고친다.

**비용** — 무엇이 바뀌는지 로그로 찍는 임시 훅과, 데이터 흐름(렌더 중 setState·새 참조 의존성)을 고치는 리팩터.

**측정** — 의존성 변경 로그(어느 키가 바뀌나), 수정 후 렌더 수.
...
**문제** — 앱을 몇 시간 켜 두면 느려지고 탭 메모리가 계속 오른다. 어디서 새는지 모른다.

**대안** — 의심 가는 코드를 고치고 '좋아진 것 같다'고 끝내면 증명이 없다. 페이지 새로고침을 안내하는 건 회피다.

**기준** — 증가를 증명했는가(스냅샷 2회 비교)로 시작한다.

**결론** — 힙 스냅샷 비교와 반복 시나리오로 증가를 확정한 뒤 retainer 로 원인을 찾고 같은 시나리오로 재측정한다.

**비용** — 반복 시나리오와 스냅샷 비교를 유지해야 하고, 클린업 코드가 늘어난다.

**측정** — 동작 전후 힙 스냅샷 차이, retainer 경로, 수정 후 같은 시나리오 재측정.
...
**문제** — Safari 에서만 레이아웃이 깨진다. 다른 브라우저는 정상이고 어느 속성이 문제인지 모른다.

**대안** — UA 로 Safari 만 분기하면 다음 버전에서 다시 깨지고, 벤더 접두사를 무작위로 붙이는 것도 원인 파악이 아니다.

**기준** — 기능 지원 여부를 확인했는가, UA 로 분기하나 기능 탐지하나로 나눈다.

**결론** — 지원 표 확인 → 최소 재현 → @supports/기능 탐지 폴백. UA 스니핑은 마지막 수단.

**비용** — 지원 표 확인·최소 재현·기능 탐지 폴백을 만드는 시간. 지원 정책에 따라 아예 폴백만 제공할 수도 있다.

**측정** — 최소 재현 페이지, @supports 분기가 실제로 갈리는지 각 엔진에서 확인.
...
**문제** — 로컬·스테이징은 정상인데 프로덕션에서만 버그가 난다. 코드는 같은 커밋이다.

**대안** — '같은 코드니 문제 없다'로 닫거나, 프로덕션에 console.log 를 뿌려 추측하는 건 미해결이다.

**기준** — 서빙되는 산출물이 로컬과 같은가부터 확인한다.

**결론** — 번들 해시·API 호스트·플래그·빌드 모드·캐시를 사슬로 실측한다. '같은 코드니 문제 없다'는 결론이 아니다.

**비용** — 서빙 산출물·환경 변수·플래그·캐시·소스맵을 하나씩 대조하는 절차와 프로덕션 유사 빌드를 로컬에 띄우는 시간.

**측정** — 응답 HTML 의 번들 해시 vs 로컬 빌드 해시, version.json, 플래그 값, 캐시 우회 요청 결과.
...
**문제** — 로딩 중 콘텐츠가 위아래로 밀려 사용자가 잘못 클릭한다. 이미지·폰트·배너 중 무엇이 원인인지 모른다.

**대안** — 전환 애니메이션으로 부드럽게 보이게 하는 건 이동 자체를 없애지 않는다. 스켈레톤도 높이가 다르면 이동 원인이 된다.

**기준** — 크기를 모르는 자원이 나중에 도착하는가로 본다.

**결론** — 자리 확보(aspect-ratio·최소 높이)·폰트 메트릭 정렬·삽입 영역 예약. 이동 요소는 attribution 으로 특정한다.

**비용** — 모든 자원의 크기를 미리 지정하고 삽입 영역을 예약하는 마크업 규율, 폰트 메트릭 정렬.

**측정** — CLS attribution 의 이동 요소·시각, 수정 후 CLS p75.
...
**문제** — PR diff 가 왔다. 시간은 20분이고 어디부터 볼지 정해야 한다.

**대안** — 위에서 아래로 스타일부터 읽으면 계약 변경·보안 결함을 놓친다. 전부 같은 무게로 코멘트하면 저자가 무엇이 차단인지 모른다.

**기준** — 틀리면 피해가 나는 것부터 보는가로 순서를 정한다.

**결론** — 계약 변경·정확성·보안 → 실패 경로 → 테스트가 변경을 잡는가 → 가독성. 스타일은 자동화.

**비용** — 차단/제안 라벨 규칙과 스타일 자동화 도구를 팀에 도입해야 한다.

**측정** — 차단 지적 비율, 리뷰 후 프로덕션 결함 유입, 스타일 코멘트 수(자동화 후 0 이어야).
...
**문제** — 댓글 본문을 dangerouslySetInnerHTML 로 렌더하는 diff 다. 서식 표시가 필요하다는 이유다.

**대안** — 'sanitize 하면 됩니다' 한 줄은 어떤 sanitizer·어떤 설정인지 검증이 없고, CSP 가 있으니 괜찮다는 것은 DOM 오염을 못 막는다.

**기준** — 사용자 입력이 HTML 로 실행되는가로 본다.

**결론** — 차단. 텍스트 렌더 또는 허용 목록 sanitizer. CSP 는 대체가 아니라 2차 방어.

**비용** — 허용 목록 sanitizer 도입·유지, 마크다운 경로로 바꾸면 렌더러 교체 비용.

**측정** — XSS 스캐너, 스크립트·이벤트 핸들러·javascript: 스킴 입력 케이스 테스트.
...
**문제** — 목록 행마다 useEffect 에서 상세 API 를 호출하는 diff 다. 행이 100개면 요청 100개다.

**대안** — 무조건 차단하면 항목이 항상 5개인 화면에선 과잉이고, 그냥 통과시키면 대량 목록에서 요청 폭주다.

**기준** — 요청 수가 항목 수에 비례하는가와 그 규모가 실제인가로 본다.

**결론** — N+1 이면 배치·목록 필드 포함으로. 불가피하면 동시성 제한·취소·캐시. 규모가 작으면 과잉 지적이다.

**비용** — 배치 엔드포인트를 서버 팀과 협상하거나, 클라이언트에서 동시성 제한·취소·캐시를 붙이는 코드.

**측정** — 행 수 대비 요청 수, 스크롤 시 취소된 요청 수, INP.
...
**문제** — div 에 onClick 을 단 '저장' 버튼 diff 다. 마우스로는 동작한다.

**대안** — '접근성 고려 필요' 같은 모호한 코멘트는 무엇을 고칠지 알려주지 않고, div 에 role·tabindex·키 핸들러를 전부 붙이라는 건 매번 그 비용을 내게 한다.

**기준** — 키보드·보조기기가 버튼으로 인식하는가로 본다.

**결론** — button 으로 교체가 가장 싸다. 이동이면 링크. 스타일 때문에 시맨틱을 버리지 않는다.

**비용** — 기본 스타일 초기화 CSS, 공용 버튼 컴포넌트가 없다면 만드는 비용.

**측정** — 키보드(Tab·Enter·Space)만으로 저장 e2e, Accessibility 트리의 role.
...
**문제** — 타입 에러를 as any 로 막은 diff 다. 저자는 '급해서'라고 한다.

**대안** — '규칙 위반' 지적만 하면 저자가 대안을 모르고, 그냥 통과시키면 회귀가 컴파일을 통과한다.

**기준** — 왜 타입이 안 맞는지 원인을 물었는가로 시작한다.

**결론** — 가드·유니온·스키마·satisfies 로 대체, 서드파티 결함은 한 곳에 격리. 급하면 좁힌 단언 + 제거 조건 이슈.

**비용** — 가드·스키마·격리 래퍼를 쓰는 코드, 급한 경우 좁힌 단언 + 제거 조건이 적힌 이슈 관리.

**측정** — any 사용 수, 후속 이슈 이행률, 그 경로의 런타임 타입 에러.
...
**문제** — 과제를 제출한다. 리뷰어는 내 환경을 모르고 시간이 10분이다.

**대안** — 기능 목록만 적으면 리뷰어가 실행에 실패했을 때 그대로 끝난다. 시간·과정 서술은 리뷰어가 보지 않는다.

**기준** — 리뷰어가 실행할 수 있는가를 최우선으로 본다.

**결론** — 실행 명령 → 범위와 안 한 것 → 설계 결정과 한계 → AI 사용과 검증. 실행 실패면 끝이다.

**비용** — README 작성 시간과 미완·가정을 솔직히 적는 부담.

**측정** — 다른 기기에서 README 만 보고 실행 성공하는지, 미완 항목이 명시돼 있는지.
...
**문제** — 과제 요구사항이 두 가지로 읽힌다. 물어볼 창구가 없거나 답이 늦다.

**대안** — 멈추고 기다리면 시간이 사라지고, 임의로 정하고 기록하지 않으면 리뷰어가 오해로 본다.

**기준** — 가정을 명시하고 영향 범위를 제한했는가로 본다.

**결론** — 가정을 적고 진행한다. 반대 해석이면 어디만 바뀌는지 설계에 반영한다. 멈추고 기다리지 않는다.

**비용** — 가정을 문서화하고 반대 해석에도 수정 범위가 작게 설계하는 추가 사고.

**측정** — 제출물의 가정 목록, 반대 해석으로 바꿀 때 변경 파일 수.
...
**문제** — 포트폴리오 프로젝트의 기술 선택 이유를 묻는다. 당시엔 튜토리얼을 따랐다.

**대안** — '인기 있어서'는 감점이고, 없는 비교를 지어내면 후속 질문에서 무너진다.

**기준** — 제약에서 출발해 대안을 비교했는가로 본다.

**결론** — 문제·제약 → 대안 → 선택 → 지금이라면. '인기 있어서'는 감점, 학습 목적이면 그렇게 말한다.

**비용** — 제약·대안·비용을 사후에 정리하는 시간, 학습 목적이었다면 그렇게 말하는 솔직함.

**측정** — 면접관의 후속 질문에 대안 비교로 답할 수 있는지.
...
**문제** — 과제에 테스트를 넣어야 하는데 시간이 부족하다. 무엇을 얼마나 쓸지 정해야 한다.

**대안** — 스냅샷을 전량 찍거나 커버리지 숫자를 채우는 건 리뷰어에게 아무 신호가 아니다. 아예 빼면 판단 근거가 없다.

**기준** — 무엇을 막는 테스트인지 설명할 수 있는가로 본다.

**결론** — 핵심 로직 유닛 + 흐름 하나 통합, 요구사항 문장 이름, 접근성 쿼리. 없어도 계획은 README 에.

**비용** — 핵심 로직·흐름 하나·실패 경로 하나를 고르는 판단과 작성 시간.

**측정** — 테스트 이름이 요구사항 문장인지, 실패 경로 테스트가 있는지.
...
**문제** — 과제 후속 인터뷰다. 내 코드의 결정을 방어하고 확장 질문에 답해야 한다.

**대안** — 완벽하다고 방어하면 약점 지적에 무너지고, 자기 코드 구조를 기억 못 하면 신뢰를 잃는다.

**기준** — '왜 이렇게 했나'와 '요구가 바뀌면 어디를 고치나'에 답할 수 있는가로 준비한다.

**결론** — 결정 지점과 약점을 먼저 정리하고, 규모 10배에서 먼저 깨질 곳을 지목할 수 있게 한다.

**비용** — 제출 후 결정 지점·약점·규모 10배 병목을 정리하는 시간.

**측정** — 약점을 스스로 먼저 말할 수 있는지, 확장 시 어디가 먼저 깨지는지 지목 가능한지.
...
**문제** — AI 도구로 코드를 많이 만든다. 어디까지 맡기고 어디서 직접 판단할지 기준이 없다.

**대안** — 전부 맡기면 경계·보안 결함이 그대로 들어가고, 전혀 안 쓰면 보일러플레이트에 시간을 쓴다.

**기준** — 틀렸을 때 비용과 검증 가능성으로 위임 범위를 정하는가로 본다.

**결론** — 보일러플레이트·테스트 초안은 맡기고 경계·보안·비가역 변경은 직접. 책임은 제출자에게.

**비용** — 팀 규칙(표기·검증·금지 영역) 수립과 리뷰 책임 유지.

**측정** — 생성 코드 결함률, 리뷰 왕복 수, 비가역 변경에 생성 코드가 들어간 사례.
...
**문제** — AI 가 만든 코드가 테스트를 통과했다. 그래도 존재하지 않는 API·빠진 실패 경로가 섞여 있을 수 있다.

**대안** — '테스트 통과면 된다'는 테스트가 해피 패스만 보는 경우 환각 API 를 놓치고, 전부 다시 쓰는 건 도구를 쓰는 의미가 없다.

**기준** — 요구 대조 → 실패 경로 실행 → 문서로 존재 확인 순서가 있는가로 본다.

**결론** — 테스트 통과로 끝내지 않는다. 환각 API 는 문서로 걸러내고 컨벤션에 맞춘다.

**비용** — 요구 대조·실패 경로 실행·공식 문서 확인에 드는 시간.

**측정** — 문서에 없는 API 검출 수, 실패 경로 테스트 추가 수, 컨벤션 위반 수.
...
**문제** — 그럴듯한 코드를 받았다. 결함을 찾아 왜 생기는지·어떤 입력에서 드러나는지 설명해야 한다.

**대안** — 스타일만 지적하거나 실행하지 않고 넘어가면 결함이 남는다. 전부 다시 쓰는 건 검토 능력을 보이지 못한다.

**기준** — 계약(입력·출력·에러·순서)부터 확인하고 재현 입력을 대는가로 본다.

**결론** — await 누락·off-by-one·얕은 복사·느슨한 비교·실패 미처리 순으로 찾고 수정 + 테스트로 닫는다.

**비용** — 계약 확인·재현 입력 작성·수정 후 테스트 작성 시간.

**측정** — 결함별 재현 입력과 수정 후 통과하는 테스트.
...
**문제** — 팀에 AI 코딩 도구를 도입한다. 어떤 데이터를 보내도 되는지, 생성 코드를 어떻게 검증할지 정해야 한다.

**대안** — 도입 자체를 성과로 보고 규칙 없이 열면 시크릿·고객 데이터가 외부로 나가고, 게이트를 우회하는 예외 경로가 생긴다.

**기준** — 데이터 경계와 검증 게이트가 있는가로 본다.

**결론** — 보낼 수 있는 데이터를 정하고 생성 코드도 같은 리뷰·테스트·스캔을 통과시킨다. 효과는 리드타임·결함률로 측정한다.

**비용** — 데이터 경계·검증 게이트·라이선스 검토·측정 인프라 운영.

**측정** — 리드타임·결함률 전후, 데이터 경계 위반 사례, 리뷰 부담 변화.
...
**문제** — 토스 코딩 테스트는 프로그래밍·서술형·객관식 세 파트다. 시간이 부족하고 파트별 평가 기준이 다르다.

**대안** — 프로그래머스식 알고리즘 속도만 연습하면 출제진이 명시한 '의도 파악·확장성'과 서술형 실무 경험을 못 채운다.

**기준** — 파트별로 무엇을 평가하는지와 배점 대비 시간을 어떻게 쓰는지로 나눈다.

**결론** — 출제진 기준은 '입사 즉시 프로젝트 수행 역량'. 의도 파악·확장성·실무 경험 서술이 점수고, 확실한 문제부터 푼다.

**비용** — 실무 경험을 원인·측정·해결로 서술하는 준비와 배점 대비 시간 배분 연습.

**측정** — 파트별 득점, 마킹 후 재방문한 문항 수, 서술형에 수치·과정이 들어갔는지.
...
**문제** — 토스 과제에 실패하는 테스트 코드가 주어졌다. 제한 시간 안에 통과시키고 코드 완성도도 보여야 한다.

**대안** — 테스트를 통과시키려고 구현을 끼워 맞추면 리뷰에서 무너지고, 한 테스트 디버깅에 매달리면 다른 문제를 못 푼다(합격자 회고).

**기준** — 테스트를 명세로 읽는가와 시간 박스가 있는가로 본다.

**결론** — 실패 지도 → 우선순위 → 핵심 흐름 → 리팩터. 테스트 디버깅에 갇히지 않고 미완·가정을 제출물에 적는다.

**비용** — 테스트를 명세로 번역하는 시간, 시간 박스 규율, 미완·가정 기록.

**측정** — 통과 테스트 수, 남긴 미완 목록, 리뷰어가 읽기 쉬운 구조인지.
...
**문제** — 1차 면접에서 면접관이 내 과제의 개선점을 하나씩 짚는다. 놓쳤던 부분도 있다.

**대안** — 방어하거나 '시간이 없어서'로 끝내면 판단 과정을 보여 줄 기회를 잃고, 무조건 동의하면 근거가 없어 보인다.

**기준** — 방어하나, 근거와 대안을 내나로 갈린다.

**결론** — 재진술 → 당시 근거 → 인정과 대안 → 트레이드오프. 모르는 건 모른다고 하고 아는 곳까지 말한다.

**비용** — 제출 후 아쉬운 점을 정리하고 리팩터해 보는 준비 시간.

**측정** — 지적마다 근거·대안·트레이드오프를 말했는지, 모르는 것을 모른다고 했는지.
...
**문제** — 라이브 코딩에서 AI 자동 완성을 끄라고 했다. 문제 의도가 불명확하고 시간은 20분이다.

**대안** — 조용히 타이핑만 하면 판단 과정이 안 보이고, 의도 확인 없이 풀면 다른 문제를 푼다. 시간 안에 못 끝낼 때 아무 말 없이 멈추는 것도 감점이다.

**기준** — 의도를 확인하고 판단을 소리 내는가로 본다.

**결론** — 역질문 → 경계 조건 → 단순 버전 → 에러 처리. 못 끝내면 의사코드와 말로 마감한다. 조용한 타이핑은 감점이다.

**비용** — 역질문·경계 조건·의사코드로 말하며 코딩하는 연습. 자동 완성 없이 API 시그니처를 기억하는 부담.

**측정** — 경계 조건을 스스로 꺼냈는지, 막힌 지점에서 판단을 말로 남겼는지, 미완을 의사코드로 채웠는지.
...
**문제** — 이력서에 Next.js 를 적었다. 면접관이 '왜 썼나'를 묻는다. 당시엔 팀 표준이라 썼다.

**대안** — '표준이라서·인기라서·빠르다고 해서'는 토스 면접관이 직접 감점 신호로 든 답이다. 쓰지 않았을 경우를 설명 못 하면 도구에서 출발한 답이다.

**기준** — 해결하려던 문제에서 출발하는가, 도구에서 출발하는가로 갈린다.

**결론** — 문제 → 대안 → 선택 이유 → 비용 → 측정. 인기·관성은 근거가 아니다. 토스뱅크는 측정 후 App Router 를 도입하지 않았다.

**비용** — 해결하려던 문제·대안·비용·측정을 사후에 정리하는 시간. 도입하지 않는 결정을 정당한 결과로 다루는 태도.

**측정** — 후속 질문 '없이 만들면 무엇이 달라지나', 'App Router 로 올릴 건가'에 측정 근거로 답할 수 있는지.
...
**문제** — 기술 면접에서 상태 관리 라이브러리를 설명해 달라고 한다. 이름은 알지만 개념 정리가 안 됐다(합격 수기의 실제 상황).

**대안** — Redux·Zustand·Jotai 이름과 인상 비평만 나열하면 '개념 정리 안 됨'으로 보인다. 서버 데이터를 전역 스토어에 넣는 답은 캐시 문제를 모르는 것.

**기준** — 이름을 나열하나 모델(스토어/원자/훅 selector/서버 캐시)로 분류하나로 갈린다.

**결론** — 모델로 나누고 선택 기준은 상태의 수명과 독자. 서버 사본은 전용 계층, atom 은 UI·파생 상태.

**비용** — 모델(스토어/원자/훅 selector/서버 캐시)로 분류하고 선택 기준(수명·독자)을 세우는 학습.

**측정** — 분류 기준과 각 모델의 부분 구독·파생 상태 처리 차이를 말했는지, Jotai atom 을 설명할 수 있는지.
...
**문제** — 2차 면접에서 '예측 불가 트래픽 급증'과 '레거시 기술 부채' 시나리오를 묻는다. 직접 겪은 경험이 없을 수 있다.

**대안** — '서버가 알아서'나 '전면 재작성'은 프론트가 통제할 축을 모르는 답이다. 경험이 없다고 침묵하는 것도 답이 아니다.

**기준** — 프론트가 통제할 수 있는 축(캐시·요청량·플래그·저하)을 나누는가로 본다.

**결론** — 트래픽은 재시도 폭주 차단·비핵심 지연·플래그 차단, 부채는 측정→Strangler→기한. '서버가 알아서'는 답이 아니다.

**비용** — 프론트가 할 수 있는 것(캐시·요청량·플래그·저하)과 부채 이관 절차(측정→Strangler→기한)를 설계 원칙으로 준비하는 시간.

**측정** — 시나리오별로 첫 조치·되돌릴 수 있는지·측정할 지표를 말했는지.
...
**문제** — 컬처 면접에서 동료 피드백 사례를 묻는다. 면접관은 소통 방식을 채용 사유로 들었다.

**대안** — '좋게 말한다' 같은 태도 표현만 있으면 사례가 없고, 사람에 대한 평가를 말하면 사실 기반이 아니다.

**기준** — 사람이 아니라 사실과 영향에 대해 말하는가로 본다.

**결론** — 상황 → 관찰한 사실 → 영향 → 제안 → 상대 반응. 저맥락자도 이해하는 리뷰 코멘트가 그 사례다.

**비용** — 실제 사례를 상황·사실·영향·제안·결과로 정리하는 준비.

**측정** — 면접관이 후속으로 '상대가 동의하지 않았을 때'를 물었을 때 답이 이어지는지.
...
**문제** — 코드 리뷰에서 '이 코드가 왜 나쁜가'를 설명해야 하는데 팀마다 기준이 다르다. 토스는 FF 4기준을 공개했다.

**대안** — '클린 코드'·'DRY' 같은 구호는 기준 간 충돌(중복 허용 vs 응집)을 설명하지 못한다.

**기준** — '좋은 코드'를 변경 용이성으로 정의하고 기준 간 충돌을 인정하는가로 본다.

**결론** — 변경하기 쉬운 코드. 가독성 > 예측 가능성 > 응집도 > 결합도이고, 기준은 서로 충돌하므로 상황별로 판단한다.

**비용** — FF 4기준을 자기 코드에 적용해 충돌 사례를 찾는 학습.

**측정** — 4기준을 우선순위와 함께 말하고, 자기 코드에서 충돌한 사례를 하나 댈 수 있는지.
...
**문제** — 프로젝트가 커지면서 파일 하나 고치는데 폴더 다섯 개를 건너다닌다. 공용 컴포넌트엔 불리언 prop 이 여덟 개다.

**대안** — components/hooks/utils 종류별 폴더는 함께 바뀌는 파일을 흩어 놓고, 변경을 예측한 조기 추상화는 분기만 늘린다.

**기준** — 함께 바뀌는 단위로 묶는가, 종류로 묶는가로 갈린다.

**결론** — 함께 수정되는 파일을 같은 폴더에. 추상화는 변경을 예측해 미리 하지 않고 세 번째 반복에서, 그전엔 중복을 허용한다.

**비용** — 도메인 단위로 폴더를 재편하고 조기 추상화를 되돌리는 리팩터 비용. 중복을 허용하는 판단.

**측정** — 한 기능 수정에 건드리는 폴더 수, 공용 컴포넌트의 props·분기 수 추세.
...
**문제** — 화면마다 isLoading·error 분기가 성공 경로를 덮어 코드가 길고, 실패 처리가 어디선가 빠진다.

**대안** — 컴포넌트마다 분기를 두는 현재 방식은 반복과 누락을 만들고, 전역 로딩 스피너 하나로 덮는 건 어느 부분이 늦는지 숨긴다.

**기준** — 성공 경로와 실패 처리가 한 컴포넌트에 섞여 있는가로 본다.

**결론** — 성공만 그리고 로딩·에러는 Suspense/Error Boundary 경계가 맡는다. 경계 위치와 reset 전략이 설계의 본체다.

**비용** — Suspense/Error Boundary 경계 위치와 reset 전략을 설계하는 비용. 경계를 잘못 두면 깜빡임·중복 요청.

**측정** — 경계별 fallback 표시 빈도, 중복 요청 수, 실패 처리 누락 화면 수.
...
**문제** — 폼 유효성 실패를 throw 로 던졌더니 Error Boundary 가 필드 메시지를 화면 전체 에러로 바꾼다.

**대안** — 전부 throw 는 예상된 결과를 예외로 만들고, 전부 값으로 돌리면 네트워크·권한 실패까지 호출부가 일일이 검사해야 한다.

**기준** — 예상된 결과인가 예외적 실패인가로 나눈다.

**결론** — 유효성 실패는 값(유니온)으로, 네트워크·권한 실패는 throw 로. 두 채널을 섞으면 경계가 필드 메시지를 전체 에러로 바꾼다.

**비용** — 판별 유니온 반환 타입 설계와 서버 400 응답을 필드 에러 값으로 변환하는 코드.

**측정** — 유효성 실패가 필드 옆에 표시되고 경계로 새지 않는지, 예외는 경계가 잡는지 e2e.
...
**문제** — 모달·바텀시트마다 useState(false) 와 onClose prop 이 반복되고, 확인 결과를 호출부로 돌리려면 콜백을 몇 단계 내려야 한다.

**대안** — Context 하나에 모든 모달 상태를 두면 소비자 전체가 리렌더되고, prop drilling 은 결합도를 올린다.

**기준** — 여는 곳과 닫는 곳이 한 자리에 있는가로 본다.

**결론** — 호출부에서 열고 Promise 로 결과를 받는다. 트리 밖 렌더라 포커스·접근성은 직접 책임진다.

**비용** — OverlayProvider 도입과 트리 밖 렌더의 포커스·접근성을 직접 책임지는 비용.

**측정** — 호출부 코드 줄 수, 포커스 복귀·트랩 e2e.
...
**문제** — 계좌 개설 퍼널이 10단계다. 단계마다 필요한 값이 다르고 뒤로 가기·새로고침·이탈 후 재진입이 있다.

**대안** — 단계 인덱스 하나와 전 필드 optional 인 거대 폼 state 는 어느 단계에서 값이 채워졌는지 타입이 말해 주지 않아 undefined 방어가 곳곳에 생긴다.

**기준** — 단계 전이가 타입으로 강제되고 히스토리로 복원되는가로 본다.

**결론** — 단계별 필수 상태를 타입으로 좁히고 히스토리 기반으로 뒤로 가기를 처리한다. 거대한 optional 폼 state 는 안티패턴.

**비용** — 단계별 타입 정의와 히스토리 기반 상태 관리 학습, 서버 저장 시점 결정.

**측정** — 뒤로 가기 후 값 복원 e2e, 필수 값 누락이 컴파일 에러로 잡히는지.
...
**문제** — lodash 를 통째로 import 하는 프로젝트의 번들이 크다. 대체 라이브러리로 바꾸자는 제안이 나왔다.

**대안** — '가볍다고 들어서' 바꾸는 건 근거가 없고, lodash-es 로만 바꾸면 구현 무게는 그대로다.

**기준** — 작아지는 이유를 구조(ESM·함수 모듈·sideEffects)로 설명하는가로 본다.

**결론** — 트리 셰이킹이 가능한 구조라서 쓰는 함수만 남는다. 전후는 번들 분석과 필드 지표로 증명한다.

**비용** — 마이그레이션 작업, compat 레이어를 거쳐 걷어내는 두 단계, 동작 차이 검증.

**측정** — 번들 분석기의 전후 크기와 모듈 단위 포함 여부, 필드 LCP.
...
**문제** — 토스 웹 화면은 앱 안 웹뷰에서 돈다. 네이티브 기능(인증·결제)을 브릿지로 호출해야 하고 첫 로딩이 느리다.

**대안** — window.postMessage 한 줄로 만들면 응답 매칭·타임아웃·앱 밖 실행 시 조용한 실패가 생긴다(토스가 실제로 겪은 함정). 웹 성능 최적화만 하면 엔진 차이를 놓친다.

**기준** — 브릿지 실패가 드러나는가와 엔진별로 측정했는가로 본다.

**결론** — id 기반 왕복 + 타임아웃 + 환경 판별로 조용한 실패를 없앤다. 웹뷰 성능은 WebKit/Chromium 각각 실측한다.

**비용** — id 기반 프로토콜·타임아웃·환경 판별 구현, WebKit/Chromium 각각 측정하는 비용.

**측정** — 브릿지 타임아웃·미지원 로그, 엔진별 FCP/LCP.
...
**문제** — 홈 화면·어드민처럼 배포 없이 자주 바뀌어야 하는 화면이 있다. 운영자가 구성을 바꿀 수 있어야 한다.

**대안** — 화면마다 코드 배포는 느리고, 전부 SDUI 로 가면 복잡한 상호작용·타입 안전·디버깅이 어려워진다.

**기준** — 배포 없이 바뀌어야 하는 화면인가, 상호작용이 복잡한가로 나눈다.

**결론** — 운영자가 조합하는 단순 화면은 SDUI, 복잡한 상호작용은 아니다. 스키마 버전·폴백·관측이 필수다.

**비용** — 컴포넌트 스키마 버전·하위 호환 폴백·프리뷰 도구·관측을 유지하는 비용.

**측정** — 구성 오류율, 미지원 컴포넌트 폴백 발생 수, 렌더 성능.
...
**문제** — 100명 넘는 엔지니어가 쓰는 모노레포에서 서비스마다 React·라이브러리 버전이 달라 설치가 1분 넘고 공통 라이브러리가 어디서 깨질지 모른다(토스 실제 상황).

**대안** — 폴리리포로 쪼개면 설치는 빨라지지만 파편화·공통 코드 비용은 그대로이고 오히려 심해진다(토스가 기각한 안).

**기준** — 문제가 리포 크기인가 버전 파편화인가로 정확히 짚는가로 본다.

**결론** — 파편화다. 폴리리포는 해결이 아니고, 카탈로그로 표준 버전을 한 곳에 정의해 설치·테스트·업그레이드 경로를 통일한다.

**비용** — 표준 버전(카탈로그) 정의, 예외 승인 절차, 메이저 업그레이드 시 코드모드·호환 레이어 제공.

**측정** — 설치 시간, 사용 중인 버전 종류 수, 플랫폼 라이브러리 업데이트 소요.
...
**문제** — Pages Router 가 유지보수 모드라 다음 기술을 정해야 한다. App Router/RSC 가 후보다.

**대안** — 커뮤니티 인기·공식 권장을 근거로 도입하면 우리 화면에서 이득이 없는데 전환 비용만 낼 수 있다.

**기준** — 장점이 우리 화면에서도 유효한가와 전환·운영 비용을 감당하는가를 측정했는가로 본다.

**결론** — 직접 재본다(예: WebKit FCP 5,065ms). 이득이 없으면 도입하지 않는 것도 정당한 결론이고 ADR 로 남긴다.

**비용** — 우리 화면에서 스트리밍·번들·경계를 직접 측정하는 시간, 도입하지 않을 때 ADR 로 기록.

**측정** — 엔진별 FCP(토스 실측: Chromium 64ms vs WebKit 5,065ms), 클라이언트 번들 변화, 경계 그을 자리 수.
...
**문제** — 광고 대시보드에서 필터·정렬을 바꿀 때마다 서버를 기다리고, 서버가 지원하지 않는 정렬은 못 한다(토스 실제 상황).

**대안** — '데이터가 많으면 서버' 일반론으로 두면 네 가지 문제가 서버 스펙 추가로 하나씩 막히고, 상한 확인 없이 브라우저로 옮기면 언젠가 깨진다.

**기준** — 데이터 상한을 숫자로 확정할 수 있는가로 갈린다.

**결론** — 확정되면 브라우저(워커+Transferable), 무한히 늘면 서버. '클라이언트 필터링이 좋은가'는 질문이 아니다.

**비용** — 워커·Transferable·정렬 잠금 구현, 데이터 상한 감시와 초과 시 서버로 되돌릴 경로.

**측정** — 조작당 응답 시간, 상한 대비 실제 건수, 워커 연산 시간.
...
**문제** — PR 이 빠르게 쌓이고 리뷰는 LGTM 만 남는다(토스 모닥불 EP.12 의 문제 제기).

**대안** — '리뷰 열심히 하자' 다짐은 유지되지 않고, 리뷰어를 늘리는 것만으로는 코멘트 질이 오르지 않는다.

**기준** — 리뷰 부담을 다짐으로 줄이나 구조로 줄이나로 갈린다.

**결론** — 코멘트 형식(무엇·왜·대안·차단 여부) + 공용 기준(FF) + 워킹그룹·지표. 저맥락자가 이해하는 리뷰가 목표다.

**비용** — 코멘트 형식 규칙·공용 기준(FF)·워킹그룹 운영과 리뷰 지표 수집.

**측정** — 리뷰 지연 시간, LGTM 만 남긴 비율, 리뷰 후 결함 유입.
...
**문제** — 프론트 테스트를 어디까지 써야 하는지 팀이 합의가 없다. 토스 과제는 TDD 형태다.

**대안** — 커버리지 숫자를 목표로 하면 가짜 안심이고, '시간 없어서 안 쓴다'는 금융 서비스에서 실패 경로가 무방비다.

**기준** — '테스트를 쓰는가'가 아니라 '무엇을 왜 막는가'로 본다.

**결론** — 실패가 사용자 피해가 되는 것부터: 금액·상태 전이·권한은 유닛, 핵심 흐름은 소수 e2e. 금융은 실패 경로 우선.

**비용** — 계층 배치 합의와 실패 경로 테스트 우선 규율, 테스트 유지 비용.

**측정** — 실패 경로 테스트 비율, 장애 재발률, 리팩터 후 깨진 테스트 수.
...
**문제** — 팀에서 함수형 프로그래밍을 얼마나 쓸지 의견이 갈린다(EP.10 시청자 질문).

**대안** — '부작용이 없어 좋다' 교과서 문장은 판단 기준이 아니고, 커링·포인트프리 전면 도입은 가독성을 해친다.

**기준** — 패러다임이 아니라 어떤 문제를 줄이는가로 본다.

**결론** — 불변·순수 함수가 리렌더 판정·테스트·트리 셰이킹을 쉽게 한다. 가독성을 해치면 루프로 돌아간다.

**비용** — 불변 갱신·순수 함수 분리 규칙과 가독성 기준을 팀이 합의하는 비용.

**측정** — 리렌더 판정 정확도, 테스트 용이성, 리뷰에서 가독성 지적 수.
...
**문제** — 금액 합산·비교 화면에서 1원이 어긋나는 제보가 들어왔다. 0.1 + 0.2 !== 0.3.

**대안** — toFixed(2) 반올림은 부동소수 자체를 그대로 두고, 문자열 조작으로 포맷하면 로케일·통화에서 깨진다.

**기준** — 계산 표현과 표시 표현을 분리하는가로 본다.

**결론** — 금액은 최소 단위 정수로 계산하고 표시 직전에만 Intl.NumberFormat. 부동소수 비교는 금지, 큰 값은 BigInt.

**비용** — 금액을 최소 단위 정수로 다루는 서버 계약 변경, 큰 값은 BigInt·십진 라이브러리.

**측정** — 금액 합산·비교 테스트(경계값 포함), Intl 포맷 결과, 서버 계약 검증.
...
**문제** — 계좌번호·주민번호가 URL·로그·에러 트래킹·세션 리플레이에 새는 사고가 났다.

**대안** — '조심해서 안 찍는다'는 개인 주의는 새 코드에서 반복되고, 로그만 막으면 URL·리플레이 경로가 남는다.

**기준** — 민감 값이 흐르는 경로를 전수로 나열했는가로 본다.

**결론** — URL·로그·에러 트래킹·리플레이·분석 각 경로를 차단하고, 유틸·SDK 훅·린트로 구조적으로 강제한다.

**비용** — 마스킹 유틸·SDK beforeSend 훅·console 금지 린트·리플레이 마스킹 설정을 구조로 강제하는 비용.

**측정** — 로그·트래킹 페이로드 스캔 결과, 리플레이에서 입력값 마스킹 확인, URL 파라미터 감사.
...
**문제** — 검색창에서 'ㅅㅇ' 으로 '송금'·'수익' 을 찾아야 한다(es-hangul 이 해결하는 실제 문제).

**대안** — 초성 테이블만 하드코딩하면 자음·완성형 혼합 검색어와 NFD 입력을 처리하지 못하고, 서버 검색으로 넘기면 입력마다 왕복이다.

**기준** — 유니코드 배치 규칙으로 초성을 계산하는가로 본다.

**결론** — 음절 코드에서 초성 인덱스를 계산하고 자음·완성형 혼합 검색어를 처리한다. NFC 정규화를 먼저 한다.

**비용** — 유니코드 정규화·쌍자음 정책·조사 붙이기 같은 한글 규칙 구현 또는 라이브러리 도입.

**측정** — 혼합 검색어·NFD 입력 테스트, 검색 응답 시간.

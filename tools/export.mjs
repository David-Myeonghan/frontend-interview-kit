...
out.push("- 구조 " + Q.filter((q) => q.frame).length + "/" + Q.length + "문항 — 「문제 → 대안 → 기준 → 결론 → 비용 → 측정」 여섯 줄을 맨 위에 두고 핵심 답은 그 이유로 배치");
{
...
for (const [key, label] of Object.entries(CATS)) {
  ...
  for (const it of items) {
    ...
    if (it.struct) {
      if (it.frame) { out.push("**문제** — " + md(it.frame[0])); out.push(""); out.push("**대안** — " + md(it.frame[1])); out.push(""); }
      out.push("**기준** — " + md(it.struct[0]));
      out.push("");
      out.push("**결론** — " + md(it.struct[1]));
      out.push("");
      if (it.frame) { out.push("**비용** — " + md(it.frame[2])); out.push(""); out.push("**측정** — " + md(it.frame[3])); out.push(""); }
    }
